...
- **Escopo analisado:** `PlanetDefenseController`, `PlanetDefenseDetectable`, `PlanetDefenseEventService`, `PlanetDefenseOrchestrationService`, `RealPlanetDefensePoolRunner`, `RealPlanetDefenseWaveRunner` e o fluxo de ScriptableObjects (`DefenseEntryConfigSO`, `DefenseMinionConfigSO`, `WavePresetSo`). Referências externas observadas: `PlanetsMaster`, `PlanetsManager`, `EventBus`, `DetectionSystems`, `PoolSystem`.
- **Fluxo atual mapeado:**
  ...
  5. Configurações de minion (`DefenseMinionConfigSO`) encapsulam apenas comportamento e lifetime lógico, sem dados de pool ou prefab.【F:Assets/_ImmersiveGames/Scripts/PlanetSystems/Defense/DefenseMinionConfigSO.cs†L8-L83】
