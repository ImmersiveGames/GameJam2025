...
- **Não acontece no Soft Reset Players**:
  ...
  - Nenhuma instância é destruída/recriada; nenhum `ActorId` é regenerado.
- **O que acontece**:
...
- **Contrato estável**: reset-in-place é parte do contrato do WorldLifecycle, não workaround temporário.
