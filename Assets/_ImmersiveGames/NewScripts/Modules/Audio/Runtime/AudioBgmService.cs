...
namespace _ImmersiveGames.NewScripts.Modules.Audio.Runtime
{
    ...
    public sealed class AudioBgmService : MonoBehaviour, IAudioBgmService
    {
        ...
        public void Play(AudioBgmCueAsset cue, float fadeInSeconds = -1f, string reason = null)
        {
            ...
            var fade = ResolveFadeSeconds(fadeInSeconds);
            var hasCurrentPlayback = (_sourceA != null && _sourceA.isPlaying) || (_sourceB != null && _sourceB.isPlaying);
            ...
            if (fade > MinFadeSeconds)
            {
                ...
                targetSource.Play();
                var transitionToken = BeginTransition(TransitionKind.FadeIn, cue.name, fade, reason);
        ...
        public void Stop(float fadeOutSeconds = -1f, string reason = null)
        {
            ...
            var fade = ResolveStopFadeSeconds(fadeOutSeconds);
            ...
            var transitionToken = BeginTransition(TransitionKind.FadeOut, ActiveCue != null ? ActiveCue.name : "none", fade, reason);
        ...
        private void StartCrossfade(AudioSource fromSource, AudioSource toSource, AudioBgmCueAsset cue, AudioClip clip, float fadeSeconds, string reason)
        {
            ...
            ActiveCue = cue;
            var transitionToken = BeginTransition(TransitionKind.Crossfade, cue.name, fadeSeconds, reason);
        ...
        {
            var startAt = Time.realtimeSinceStartupAsDouble;
        ...
        private IEnumerator CrossfadeRoutine(AudioSource fromSource, AudioSource toSource, AudioBgmCueAsset targetCue, float fadeSeconds, int transitionToken)
        {
            ...
            while (Time.realtimeSinceStartupAsDouble < endAt)
            {
                ...
                if (toSource != null)
                {
                    var targetVolume = ComputeCueTargetVolume(targetCue);
        ...
        private void ApplyCurrentDuckingImmediately()
        {
            ...
            var targetVolume = ComputeCueTargetVolume(ActiveCue);
        ...
        private float ComputeCueTargetVolume(AudioBgmCueAsset cue)
        {
            ...
            var baseVolume = Mathf.Clamp01(cue.BaseVolume);
            var master = _settings != null ? Mathf.Clamp01(_settings.MasterVolume) : 1f;
            var bgm = _settings != null ? Mathf.Clamp01(_settings.BgmVolume) : 1f;
            var category = _settings != null ? Mathf.Max(0f, _settings.BgmCategoryMultiplier) : 1f;
            var ducking = _pauseDuckingEnabled ? ResolvePauseDuckingScale() : 1f;
        ...
        private void CancelActiveTransition(string cancelCause, string triggerReason)
        {
            ...
            var elapsed = Time.realtimeSinceStartupAsDouble - _activeTransition.StartedAt;
            ...
            DebugUtility.LogVerbose(typeof(AudioBgmService),
                $"[Audio][BGM] {snapshot.Kind} canceled token={snapshot.Token} cue='{snapshot.CueName}' configured={snapshot.ConfiguredSeconds:0.###} elapsed={elapsed:0.###} cause='{cancelCause}' transitionReason='{snapshot.Reason}' triggerReason='{SafeReason(triggerReason)}'.",
                DebugUtility.Colors.Info);
        }
        ...
        private void FinalizeTransition(int transitionToken, string status, string detail)
        {
            ...
            DebugUtility.LogVerbose(typeof(AudioBgmService),
                $"[Audio][BGM] {snapshot.Kind} {status} token={snapshot.Token} cue='{snapshot.CueName}' configured={snapshot.ConfiguredSeconds:0.###} elapsed={elapsed:0.###} detail='{detail}' reason='{snapshot.Reason}'.",
                DebugUtility.Colors.Info);
        }
