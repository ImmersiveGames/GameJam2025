...
using UnityEngine;
namespace _ImmersiveGames.NewScripts.Modules.GameLoop.EndConditions
{
    /// <summary>
    ...
    public sealed class GameplayOutcomeConditionsController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                _requested = true;
                _endRequest.RequestRunEnd(timeoutOutcome, timeoutReason);
                return;
            }
        ...
        private void OnGameRunStarted(GameRunStartedEvent evt)
        {
            ...
            DebugUtility.LogVerbose<GameplayOutcomeConditionsController>(
                "[GameplayOutcomeConditionsController] Rearmed on GameRunStartedEvent.",
        ...
        private void ResetLocalState(string reason)
        {
            ...
            DebugUtility.LogVerbose<GameplayOutcomeConditionsController>(
                $"[GameplayOutcomeConditionsController] State reset. reason='{reason}', startTime={_startTime:0.000}.",
                DebugUtility.Colors.Info);
        }
        ...
        private bool TryResolveEndRequestService()
        {
            ...
            if (!_loggedMissingService)
            {
                ...
                DebugUtility.LogWarning<GameplayOutcomeConditionsController>(
                    "[GameplayOutcomeConditionsController] IGameRunEndRequestService não disponível (ainda). " +
