...
namespace _ImmersiveGames.NewScripts.Modules.GameLoop.Bindings.Inputs
{
    /// <summary>
    /// Captura ESC e publica eventos NewScripts para pausar/despausar.
    /// Regras:
    /// - SÃ³ atua durante uma run ativa (apÃ³s GameRunStartedEvent).
    /// - Se a run jÃ¡ terminou (GameRunEndedEvent), ESC Ã© ignorado (Victory/Defeat usam overlay prÃ³prio).
    /// - Toggle:
    ///     - Se nÃ£o pausado -> GamePauseCommandEvent(true)
    ...
    public sealed class GamePauseHotkeyController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Espelha estado local baseado nos prÃ³prios eventos do sistema.
        ...
        private void Update()
        {
            ...
            // ESC sÃ³ Ã© Pause do jogador durante run ativa (Gameplay) e antes do final da run.
        ...
        private void OnRunStarted()
        {
            _runActive = true;
            _runEnded = false;
            // NÃ£o forÃ§amos _paused aqui; ele serÃ¡ mantido pelo espelhamento de eventos.
        }

        private void OnRunEnded()
        {
            _runEnded = true;
            // Em run ended, normalmente o sistema tambÃ©m publica GamePauseCommandEvent(true)
        ...
        {
            // Pelo log vocÃª usa GamePauseCommandEvent(true).
