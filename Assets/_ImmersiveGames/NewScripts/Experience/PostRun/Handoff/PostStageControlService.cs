    ...
    public sealed class PostStageControlService : IPostStageControlService
    {
            ...
            lock (_sync)
            {
                if (_isActive)
                {
                    DebugUtility.LogWarning<PostStageControlService>(
                        $"[OBS][PostRun][Bridge] PostStageBeginIgnored reason='already_active' signature='{Normalize(context.Signature)}' scene='{Normalize(context.SceneName)}' frame={context.Frame} outcome='{context.Outcome}' reason='{Normalize(context.Reason)}'.");
                    return false;
                }
        ...
        public Task<PostStageCompletionResult> WaitForCompletionAsync(CancellationToken cancellationToken = default)
        {
            ...
            lock (_sync)
            {
                ...
                {
                    throw new InvalidOperationException("[FATAL][PostRun] WaitForCompletionAsync chamado antes de PostStageBegin.");
                }
        ...
        private bool TryFinish(PostStageCompletionKind kind, string reason)
        {
            lock (_sync)
            {
                ...
                {
                    DebugUtility.LogWarning<PostStageControlService>(
                        $"[OBS][PostRun][Bridge] PostStageFinishIgnored kind='{kind}' reason='{Normalize(reason)}'.");
                    return false;
                }
