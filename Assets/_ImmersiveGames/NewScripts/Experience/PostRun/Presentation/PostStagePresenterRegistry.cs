    ...
    public sealed class PostStagePresenterRegistry : IPostStagePresenterRegistry
    {
        ...
        public bool TryEnsureCurrentPresenter(PostStageContext context, string source, out IPostStagePresenter presenter)
        {
            ...
            if (presenters.Count > 1)
            {
                HardFailFastH1.Trigger(typeof(PostStagePresenterRegistry),
                    $"[FATAL][H1][PostRun] Multiple IPostStagePresenter components found. source='{source}' signature='{Normalize(context.Signature)}' scene='{Normalize(context.SceneName)}' presenters='{DescribePresenters(presenters)}'.");
                return false;
            }
            ...
            {
                HardFailFastH1.Trigger(typeof(PostStagePresenterRegistry),
                    $"[FATAL][H1][PostRun] IPostStagePresenter bound with mismatched signature. source='{source}' expected='{Normalize(context.Signature)}' actual='{Normalize(candidate.PresenterSignature)}' scene='{Normalize(context.SceneName)}' presenter='{candidate.GetType().FullName}'.");
                return false;
            }
            ...
            {
                HardFailFastH1.Trigger(typeof(PostStagePresenterRegistry),
                    $"[FATAL][H1][PostRun] IPostStagePresenter bound but not ready. source='{source}' signature='{Normalize(context.Signature)}' scene='{Normalize(context.SceneName)}' presenter='{candidate.GetType().FullName}'.");
                return false;
            }
            ...
            DebugUtility.Log<PostStagePresenterRegistry>(
                $"[OBS][PostRun] PostStagePresenterAdopted source='{source}' signature='{Normalize(context.Signature)}' scene='{Normalize(context.SceneName)}' presenter='{DescribePresenter(candidate)}'.",
        ...
        public void Register(IPostStagePresenter presenter, string sessionSignature)
        {
            ...
            lock (_sync)
            {
                ...
                {
                    HardFailFastH1.Trigger(typeof(PostStagePresenterRegistry),
                        $"[FATAL][H1][PostRun] More than one IPostStagePresenter attempted to register. current='{_currentPresenter.GetType().FullName}' incoming='{presenter.GetType().FullName}' currentSignature='{_currentSessionSignature}' incomingSignature='{Normalize(sessionSignature)}'.");
                    return;
                }
