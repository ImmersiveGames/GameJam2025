    ...
    public sealed class PlayerActorCommandInputHub : MonoBehaviour, IActorCommandSourceHub
    {
        ...
        public void PrepareInputBindings(PlayerInput playerInput, string context)
        {
            ...
            DebugUtility.LogVerbose(
                ...
                $"event='ActorCommandHubPrepareStarted' actorId='{_actorId}' actorInstanceRuntimeId='{_actorInstanceRuntimeId}' playerInput='{_boundPlayerInput.name}' bindingCount='{commandBindings?.Count ?? 0}' context='{NormalizeContext(context)}' source='{nameof(PlayerActorCommandInputHub)}' reason='input_bindings_prepare_started'.",
        ...
        private void Update()
        {
            ...
            {
                var binding = _resolvedBindings[index];
        ...
        private void ResolveBindingsOrFail(string context)
        {
            ...
            for (int index = 0; index < bindings.Count; index++)
            {
                ...
                if (!binding.IsConfigured)
                {
                    ...
                    DebugUtility.LogVerbose(
                        typeof(PlayerActorCommandInputHub),
                        $"event='ActorCommandBindingSkipped' actorId='{_actorId}' actorInstanceRuntimeId='{_actorInstanceRuntimeId}' bindingId='{binding.BindingId}' commandKind='{binding.CommandKind}' valueKind='{binding.ValueKind}' triggerKind='{binding.TriggerKind}' reason='unconfigured_optional_binding'.",
                        DebugUtility.Colors.Info);
                ...
                if (!binding.Enabled)
                {
                    ...
                    DebugUtility.LogVerbose(
                        typeof(PlayerActorCommandInputHub),
                        $"event='ActorCommandBindingSkipped' actorId='{_actorId}' actorInstanceRuntimeId='{_actorInstanceRuntimeId}' bindingId='{binding.BindingId}' commandKind='{binding.CommandKind}' valueKind='{binding.ValueKind}' triggerKind='{binding.TriggerKind}' reason='disabled_optional_binding'.",
                        DebugUtility.Colors.Info);
                    _lastSkippedBindingCount++;
                    continue;
                }

                if (!TryResolveAction(binding, index, out var action))
                {
                    if (binding.Required)
                    {
                ...
                var commandId = binding.ResolveCommandIdOrFail();
        ...
        {
            action = null;
            var actionId = binding.ActionReference.action.id;
            foreach (var actionMap in _boundPlayerInput.actions.actionMaps)
            {
                if (actionMap == null)
                {
                    continue;
                ...
                foreach (var candidate in actionMap.actions)
                {
        ...
        {
            action = null;
            if (!binding.TryResolveCommandId(out var commandId) ||
                !TryGetCanonicalActionName(commandId, out string actionName))
            {
                return false;
            }

            var currentActionMap = _boundPlayerInput.currentActionMap;
        ...
        private void EmitResolvedBinding(ResolvedCommandBinding binding)
        {
            ...
            if (!TryGetCommandSink(ActorCommandId.Move, out var moveSink))
            {
                return;
            }

            var value = ReadMoveValue(binding.Action, descriptor.TriggerKind);
            var bindingId = descriptor.ResolveBindingIdOrFail();
            ...
            var dispatchResult = moveSink.AcceptCommand(command);
        ...
        private void UnregisterActionSubscriptions()
        {
            ...
            {
                var subscription = _actionSubscriptions[index];
        ...
        {
            if (!descriptor.TryResolveCommandId(out var resolvedCommandId) ||
            ...
            var value = ActorCommandValue.CreateFirePrimary(true, ActorCommandTriggerKind.Pressed);
            var bindingId = descriptor.ResolveBindingIdOrFail();
            ActorCommandEnvelope command = new(
                _actorId,
                _actorInstanceRuntimeId,
            ...
            DebugUtility.LogVerbose(
                typeof(PlayerActorCommandInputHub),
                $"event='ActorCommandEmitted' actorId='{_actorId}' actorInstanceRuntimeId='{_actorInstanceRuntimeId}' commandId='FirePrimary' bindingId='{bindingId.Value}' valueKind='Button' triggerKind='Pressed' source='{nameof(PlayerActorCommandInputHub)}' reason='fire_primary_pressed'.",
                DebugUtility.Colors.Info);

            if (!TryGetCommandSink(ActorCommandId.FirePrimary, out var fireSink))
            {
                if (descriptor.Required)
                {
            ...
            var dispatchResult = fireSink.AcceptCommand(command);
            if (!dispatchResult.IsValid)
            {
                throw new InvalidOperationException($"PlayerActorCommandInputHub received invalid dispatch result for binding '{bindingId.Value}'.");
            }
            ...
            DebugUtility.LogVerbose(
                typeof(PlayerActorCommandInputHub),
                $"event='ActorCommandDispatchAccepted' actorId='{_actorId}' actorInstanceRuntimeId='{_actorInstanceRuntimeId}' commandId='FirePrimary' bindingId='{bindingId.Value}' valueKind='Button' triggerKind='Pressed' dispatchReason='{dispatchResult.Reason}' source='{nameof(PlayerActorCommandInputHub)}' reason='command_observed'.",
