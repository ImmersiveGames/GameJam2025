...
namespace _ImmersiveGames.NewScripts.Infrastructure
{
    ...
    public static class GlobalBootstrap
    {
        ...
        private static void RegisterEssentialServicesOnly()
        {
            ...
            RegisterContentSwapStartCommitBridge();
        ...
        private static void RegisterContentSwapStartCommitBridge()
        {
            if (DependencyManager.Provider.TryGetGlobal<ContentSwapStartCommitBridge>(out var existing) && existing != null)
            {
                DebugUtility.LogVerbose(typeof(GlobalBootstrap),
                    "[ContentSwapStart] ContentSwapStartCommitBridge já registrado no DI global.",
                    DebugUtility.Colors.Info);
                return;
            }

            var bridge = new ContentSwapStartCommitBridge();
            DependencyManager.Provider.RegisterGlobal(bridge);

            DebugUtility.LogVerbose(typeof(GlobalBootstrap),
                "[ContentSwapStart] ContentSwapStartCommitBridge registrado no DI global (ContentSwapCommitted -> IntroStage).",
                DebugUtility.Colors.Info);
        }
