...
namespace _ImmersiveGames.NewScripts.Core.Logging
{
    ...
    public sealed class DebugManagerConfig : MonoBehaviour
    {
        ...
        public void ApplyConfiguration()
        {
            if (settings == null)
            {
                Debug.LogWarning("[DebugManagerConfig] DebugLogSettings não atribuído; aplicação ignorada.");
                return;
            }
        ...
        {
            DebugUtility.Log(typeof(DebugManagerConfig),
                $"[DebugManagerConfig] Estado atual => Global={DebugUtility.IsGlobalDebugEnabled}, Verbose={DebugUtility.IsVerboseLoggingEnabled}, Fallbacks={DebugUtility.IsFallbacksEnabled}, RepeatedVerbose={DebugUtility.IsRepeatedCallVerboseEnabled}, DefaultLevel={DebugUtility.DefaultDebugLevel}.");
        }
    }
}
