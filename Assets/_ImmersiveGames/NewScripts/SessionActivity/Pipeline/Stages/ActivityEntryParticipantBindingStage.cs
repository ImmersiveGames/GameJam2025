...
namespace _ImmersiveGames.NewScripts.SessionActivity.Pipeline.Stages
{
    internal static class ActivityEntryParticipantBindingStage
    {
            ...
            List<SessionActivitySnapshot> snapshots)
        {
            ...
            if (playerActorMaterializationAdapter == null)
            {
                throw new ArgumentNullException(nameof(playerActorMaterializationAdapter));
            }

            if (playerActorParticipationAdapter == null)
            {
                throw new ArgumentNullException(nameof(playerActorParticipationAdapter));
            }

            actorMaterializationPlanEntries ??= Array.Empty<SessionActivityActorMaterializationPlanEntry>();

            if (sessionActorRuntimeStore == null)
            {
                throw new ArgumentNullException(nameof(sessionActorRuntimeStore));
            }
            ...
            var startedIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingStarted);
            ...
            var expectedInventoryIdentity = BuildIdentity(command, SessionActivityStage.ActivitySetupStarted);
            if (!inventory.IsValid || inventory.Identity.CycleKey != expectedInventoryIdentity.CycleKey)
            {
                var failedIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingFailed);
            ...
            {
                var retainedSessionParticipationContext = ResolveSessionParticipationContextOrFail(
                    command,
                ...
                var completedAfterSkipIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingCompleted);
                StoreActivityParticipationBoundary(
                    new PlayerActivityParticipationContext(
            ...
            Dictionary<PlayerSessionParticipantId, SessionActivityActorMaterializationPlanEntry> materializationPlanByParticipantId =
                BuildMaterializationPlanMap(actorMaterializationPlanEntries);
            ...
            {
                var requirement = participantRequirements[index];
                ...
                var requestedParticipantId = requirement.SessionParticipantId;
                        ...
                        out string activityParticipationResolutionReason))
                {
                    ...
                    var missingSessionParticipantIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingFailed);
                ...
                if (activityParticipantBinding.RequiresPlayerActor)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        var missingMaterializationPlanIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingFailed);
                ...
                ActivityParticipantMaterializationCommand materializationCommand = new(
                    ...
                    command.Source,
                    command.Reason);
                if (!bindCommand.IsValid || !materializationCommand.IsValid)
                {
                    var failedIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingFailed);
                    endpoint.SetCurrentIdentity(failedIdentity, SessionActivityStage.ActivityParticipantBindingFailed);
                    endpoint.EmitFact(
            ...
            var completedIdentity = BuildIdentity(command, SessionActivityStage.ActivityParticipantBindingCompleted);
            StoreActivityParticipationBoundary(
                new PlayerActivityParticipationContext(
                    completedIdentity,
            ...
            IReadOnlyList<ActivityParticipantMaterializationCommand> materializationCommands)
        {
            ...
            {
                var bindCommand = bindCommands[index];
                endpoint.EmitFact(
                    facts,
            ...
            {
                var materializationCommand = materializationCommands[index];
                endpoint.EmitFact(
                    facts,
            ...
            Dictionary<PlayerSessionParticipantId, SessionActivityActorMaterializationPlanEntry> materializationPlanByParticipantId)
        {
            try
            {
                ...
                {
                    var materializationCommand = plan.MaterializationCommands[index];
                    var actorIdentity = EnsureParticipantMaterialized(
                        activityId,
                        identity,
                        materializationCommand,
                        playerActorMaterializationAdapter,
                ...
                {
                    var bindCommand = plan.BindCommands[index];
                    var actorIdentity = EnsureResolvedActorIdentityForActivityParticipantOrFail(bindCommand.ParticipantBinding, ensuredActorsByActivityParticipant, activityId, "bind");
                    PlayerActorParticipationEnterCommand enterCommand = new(identity, new[] { actorIdentity }, bindCommand.Source, bindCommand.Reason);
                    IReadOnlyList<PlayerActorParticipationEnterRecord> records = playerActorParticipationAdapter.Execute(enterCommand, identity, playerActorRegistry);
        ...
        private static PlayerActorRuntimeHandle EnsureResolvedPlayerActorHandleForActivityParticipantOrFail(
            ...
            string operation)
        {
            ...
            {
                throw new InvalidOperationException("Activity participant binding is invalid for player actor handle resolution.");
            }

            if (!TryResolvePlayerActorHandleForParticipant(playerActorRegistry, participantBinding.ParticipantId, out var handle) ||
                !handle.IsValid)
            {
                throw new InvalidOperationException(
                    $"Activity participant '{participantBinding.ParticipantId}' is not available for operation='{operation}' activityId='{activityId}' playerSlotId='{participantBinding.PlayerSlotId}' actorDefinitionId='{participantBinding.ActorDefinitionId}' actorId='{participantBinding.ActorId}'.");
            }
        ...
        private static Dictionary<PlayerSessionParticipantId, SessionActivityActorMaterializationPlanEntry> BuildMaterializationPlanMap(IReadOnlyList<SessionActivityActorMaterializationPlanEntry> materializationPlanEntries)
        {
            ...
            for (int index = 0; index < materializationPlanEntries.Count; index++)
            {
                ...
                if (entry.IsValid)
                {
                    map.TryAdd(entry.ParticipantId, entry);
                }
            }
        ...
        private static PlayerActorIdentityRecord EnsureParticipantMaterialized(
            ...
            string reason)
        {
            var participant = command.ParticipantBinding;
            ...
            if (retainedLookupResult.IsResolved)
            {
                ...
                var identityComponent = retainedInstance.GetComponent<PlayerActorIdentity>();
                ...
                var reboundIdentity = BuildParticipantActorIdentity(identity, participant);
                ...
                var retainedActor = retainedInstance.GetComponent<Actor>();
            ...
            var localPosition = materializationPlanEntry.PlacementMode == ActorPlacementMode.FixedTransform
                ? materializationPlanEntry.LocalPosition
                : Vector3.zero;
            var localEuler = materializationPlanEntry.PlacementMode == ActorPlacementMode.FixedTransform
                ? materializationPlanEntry.LocalEulerAngles
                : Vector3.zero;
            var actorIdentity = BuildParticipantActorIdentity(identity, participant);
            ...
            IReadOnlyList<PlayerActorMaterializationRecord> records = playerActorMaterializationAdapter.Execute(playerMaterializationCommand, identity);
            if (records.Count != 1 || !records[0].IsValid)
            {
                throw new InvalidOperationException(
            ...
            string operation)
        {
            ...
            var sessionParticipantId = participant.ParticipantId;
            if (materializationPlanByParticipantId != null &&
                materializationPlanByParticipantId.TryGetValue(sessionParticipantId, out var byParticipant) &&
            ...
            string operation)
        {
            ...
            var participantId = participantBinding.ParticipantId;
            if (ensuredActorsByActivityParticipant == null || !ensuredActorsByActivityParticipant.TryGetValue(participantId, out var identity) || !identity.IsValid)
            {
                throw new InvalidOperationException(
            ...
            string eventName)
        {
            ...
            if (playerActorRegistry.TryGetIndexedActiveHandleByParticipant(actorIdentity.ParticipantId, out var activeHandle))
            {
            ...
            PlayerSessionParticipationContext sessionParticipationContext)
        {
            ...
            string sessionParticipationState = sessionParticipationContext is { IsValid: true }
                ? "present"
                : "absent";
            ...
            out string resolutionReason)
        {
            ...
            if (!TryResolveSessionParticipantBinding(requirement, sessionParticipationContext, out var sessionParticipant, out resolutionReason))
            {
                return false;
            }
        ...
        private static bool TryResolveSessionParticipantBinding(
            ParticipantRequirement requirement,
            PlayerSessionParticipationContext sessionParticipationContext,
            ...
            out string resolutionReason)
        {
            binding = default;
            resolutionReason = "unknown";
            if (sessionParticipationContext == null ||
                !sessionParticipationContext.IsValid ||
                sessionParticipationContext.Participants == null)
            {
            ...
            IReadOnlyList<PlayerSessionParticipantBinding> participants = sessionParticipationContext.Participants;
            ...
            List<SessionActivitySnapshot> snapshots)
        {
            ...
            var context = activityParticipationRuntimeState.CurrentSessionParticipationContext;
        ...
        private static void LogActivityParticipationBindingSkipped(
            ActivityEntryParticipantBindingCommand command,
            SessionActivityIdentity identity,
            PlayerSessionParticipationContext sessionParticipationContext,
            ...
            string skipReason)
        {
            ...
            DebugUtility.LogVerbose(
                typeof(ActivityEntryParticipantBindingStage),
                $"event='ActivityParticipationBindingSkipped' pipelineId='{command.Identity.PipelineId}' sessionStateId='{command.Identity.SessionId}' activityId='{command.ActivityId}' entrySequence='{identity.EntrySequence}' requirementId='{requirementId}' participantId='{FormatSessionParticipantId(participantId)}' sessionParticipationContext='{sessionParticipationState}' skipReason='{skipReason}' owner='ActivityEntryParticipantBindingStage' source='{command.Source}' reason='{command.Reason}'.",
                DebugUtility.Colors.Warning);
        }
