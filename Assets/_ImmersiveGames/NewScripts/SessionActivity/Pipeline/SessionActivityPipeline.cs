...
namespace _ImmersiveGames.NewScripts.SessionActivity.Pipeline
{
    public sealed class SessionActivityPipeline : ISessionActivityEntryHandoffReceiver, ISessionActivityPendingOperationCallback, ISessionActivitySnapshotPayloadProvider, IActivityEntryRuntimeBridge, IActivityEntryObjectSetupRuntimeBridge, IActivityEntryParticipantBindingRuntimeBridge, IActivityEntryActorPresentationRuntimeBridge, IActivityEntryActorParticipationRuntimeBridge, IActivityEntryPermissionTargetRuntimeBridge, IActivityEntryMovementBindingRuntimeBridge, IActivityEntryCameraBindingRuntimeBridge, IActivityExitActorTeardownRuntimeBridge
    {
        ...
        private string ResolveActivityContentReleaseContinuationKind(SessionActivityDefinition definition, int entrySequence)
        {
            ...
            if (definition is { IsValid: true, HasNextActivity: true })
            {
                return "NextActivity";
            }
            ...
            string nextStage)
        {
            ...
            bool hasInternalTransition = transition.IsValid &&
                ...
                string.Equals(transition.FromActivityId, definition.ActivityId, StringComparison.Ordinal) &&
                transition.FromEntrySequence == entrySequence;
            bool hasNextActivity = definition is { IsValid: true, HasNextActivity: true };
        ...
        {
            string routeOperationId = _lastSessionParticipationContext is { IsValid: true }
            ...
            int entrySequence)
        {
            ...
            bool IsLoadedSetForCurrentEntry(
                ...
                int currentEntrySequence)
            {
                return currentLoadedSet is { IsValid: true, Identity: { Stage: SessionActivityStage.ActivityContentLoadedSetReady } } &&
            ...
            if (targets.Count != 1 || !targets[0].IsValid)
            {
                ...
                for (int index = 0; index < targets.Count; index++)
                {
                    ...
                    if ((_activityPlayerActorRegistry.TryGetActiveHandleByParticipant(candidate.ParticipantId, out PlayerActorRuntimeHandle candidateHandle) ||
                        ...
                        candidateHandle is { IsValid: true, ActorInstanceRuntimeId: { IsValid: true } })
                    {
        ...
        private void StoreActivityParticipationContext(PlayerActivityParticipationContext context)
        {
            ...
            string sessionParticipationState = _lastSessionParticipationContext is { IsValid: true }
                ? "present"
                : "absent";
...
private bool TryBuildActivityParticipantBinding(
            ...
            out SessionActivityVisualReadinessResult result)
        {
            ...
            string observedRouteOperationId = _lastSessionParticipationContext is { IsValid: true }
                ? _lastSessionParticipationContext.RouteOperationId
                : string.Empty;
