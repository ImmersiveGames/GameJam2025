...
### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/ADR-0009-FadeSceneFlow-Evidence.md`

# Evidência — ADR-0009 (FadeSceneFlow)

**Data:** 2026-01-31  
**Fonte:** trecho de log fornecido (baseline canônico)  
**Escopo:** SceneFlow/SceneTransitionService + FadeService

## 1) Objetivo da evidência

Demonstrar, no runtime, que o envelope de transição de cenas cumpre o contrato do ADR-0009:

- FadeIn acontece antes de qualquer operação de load/unload/active.
- `ScenesReady` ocorre após as operações de cena.
- Completion gate é aguardado antes do FadeOut (quando aplicável).
- FadeOut acontece antes de `TransitionCompleted`.
- Âncoras observáveis mínimas `[OBS][Fade]` existem e carregam `signature` e `profile`.

## 2) Âncoras observadas (PASS)

### Transição id=1 (profile=startup)
...
- `[SceneFlow] TransitionCompleted`

### Transição id=2 (profile=gameplay)
...
## 3) Prova de policy (Strict/Release) instalada (PASS)

No bootstrap aparecem:

- Registro de `IRuntimeModeProvider` no escopo global.
- Registro de `IDegradedModeReporter` no escopo global.
- Log de consolidação: `[RuntimePolicy] IRuntimeModeProvider + IDegradedModeReporter registrados no DI global.`

> Observação: este trecho prova instalação da policy e do reporter. Ele **não** exercita, por si só, os casos negativos (Strict fail-fast / Release degraded).

## 4) Limites desta evidência

Este snapshot cobre **happy path** com fade disponível e executando corretamente. Para evidência “máxima” (auditoria), recomenda-se coletar mais 2 snapshots:

1) **Strict fail-fast**: fade requerido e indisponível (ex.: FadeScene fora do Build Settings ou controller ausente).  
2) **Release degraded-mode**: build Release com fade indisponível, confirmando âncora `DEGRADED_MODE feature='fade' ...`.

## 5) Anexo: trecho de log

Veja o arquivo: `ADR-0009-LogExcerpt.txt`

### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/ADR-0009-LogExcerpt.txt`
...
NEWSCRIPTS_MODE ativo: DebugUtility.Initialize ignorado.
[INFO] [DependencyBootstrapper] NEWSCRIPTS_MODE ativo: ResetStatics ignorado.
[VERBOSE] [GlobalBootstrap] NewScripts logging configured. (@ 3,34s)
<color=#00BCD4>[VERBOSE] [SceneServiceCleaner] SceneServiceCleaner inicializado. (@ 3,34s)</color>
<color=#00BCD4>[VERBOSE] [DependencyManager] DependencyManager inicializado (DontDestroyOnLoad). (@ 3,34s)</color>
[VERBOSE] [GlobalBootstrap] DependencyManager created for global scope. (@ 3,34s)
<color=#A8DEED>[VERBOSE] [GlobalBootstrap] [EventBus] EventBus inicializado (GameLoop + SceneFlow + WorldLifecycle). (@ 3,35s)</color>
...
<color=#4CAF50>[VERBOSE] [GlobalServiceRegistry] Serviço PromotionGateService registrado no escopo global. (@ 3,35s)</color>
<color=#A8DEED>[INFO] [PromotionGateInstaller] PromotionGate registrado (global).</color>
<color=#4CAF50>[VERBOSE] [GlobalServiceRegistry] Serviço IRuntimeModeProvider registrado no escopo global. (@ 3,35s)</color>
[VERBOSE] [GlobalBootstrap] Registered global service: IRuntimeModeProvider. (@ 3,35s)
<color=#4CAF50>[VERBOSE] [GlobalServiceRegistry] Serviço IDegradedModeReporter registrado no escopo global. (@ 3,35s)</color>
[VERBOSE] [GlobalBootstrap] Registered global service: IDegradedModeReporter. (@ 3,35s)
<color=#A8DEED>[VERBOSE] [GlobalBootstrap] [RuntimePolicy] IRuntimeModeProvider + IDegradedModeReporter registrados no DI global. (@ 3,35s)</color>

... (trecho omitido por brevidade; este arquivo contém apenas o excerto fornecido) ...
...
### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/Baseline2_Test-03_SceneFlow-IntroStage-LevelSwap.md`
...
### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/PATCH-ADR-0009.md`

# PATCH — ADR-0009-FadeSceneFlow.md (Adicionar referência de evidência)

> **Atenção:** este arquivo é um *patch* (snippet) para ser mesclado manualmente ao ADR-0009 existente.

## Evidências

- **2026-01-31 — Happy Path (startup + gameplay):** `Reports/Evidence/2026-01-31/ADR-0009-FadeSceneFlow-Evidence.md`

### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/PATCH-LATEST.md`

# PATCH — LATEST.md (Adicionar seção ADR-0009)

> **Atenção:** este arquivo é um *patch* (snippet) para ser mesclado manualmente ao `Docs/Reports/Evidence/LATEST.md` existente.

## ADR-0009 — FadeSceneFlow (EVIDÊNCIA 2026-01-31)

- **Status:** PASS (Happy Path)
- **Arquivo:** `Reports/Evidence/2026-01-31/ADR-0009-FadeSceneFlow-Evidence.md`
- **Fonte:** `Reports/Evidence/2026-01-31/ADR-0009-LogExcerpt.txt`
- **Âncoras presentes:** `[OBS][Fade] FadeInStarted/Completed`, `[OBS][Fade] FadeOutStarted/Completed`, `ScenesReady`, `TransitionCompleted`
- **Transições cobertas:** id=1 (startup), id=2 (gameplay)

### Fonte mesclada: `Docs/Reports/Evidence/2026-01-31/README.md`

# Evidências (2026-01-31)

Este diretório contém a evidência de runtime usada para formalizar o ADR-0009 como “contrato verificável”.

Arquivos:
- `ADR-0009-FadeSceneFlow-Evidence.md`: checklist de âncoras e conclusão (PASS).
- `ADR-0009-LogExcerpt.txt`: trecho do log (fonte primária).

Integração sugerida:
- Copiar este diretório para: `Assets/_ImmersiveGames/NewScripts/Docs/Reports/Evidence/2026-01-31/`
- Atualizar `Docs/Reports/Evidence/LATEST.md` adicionando um link/menção para esta evidência (ver `PATCH-LATEST.md`).
