# NewScripts — Raio X Arquitetural e Funcional

Status: Consolidação organizada das auditorias profundas do `NewScripts`  
Origem: auditorias isoladas do `NewScripts`, sem comparação com `com.immersive.framework`  
Modo de evidência: leitura estática / audit-only  
Data: 2026-06-21  
Uso pretendido: insumo para auditoria posterior do package atual, comparação e roadmap cíclico do Immersive Framework.

---

## 0. Como ler este documento

Este arquivo substitui os registros intermediários como **documento único de referência do `NewScripts`**.

A organização é:

1. **Raio X executivo** — visão consolidada do sistema.
2. **Mapa por escopo** — Session, Route, Activity, Local, RuntimeSpawned, Surface e CrossCutting.
3. **Inventário funcional consolidado** — capacidades agrupadas por papel arquitetural.
4. **Preservar / Redesenhar / Descartar** — catálogo de decisões para o framework novo.
5. **Dependências e ordem recomendada** — base para plano cíclico posterior.
6. **Backlog de ADRs** — decisões que precisam virar registro formal.
7. **Perguntas abertas** — dúvidas para a síntese final e comparação futura.
8. **Apêndice integral** — auditorias completas, preservadas para não perder detalhe.

Regra de interpretação:

```text
NewScripts é inventário funcional e arquitetural.
NewScripts não é arquitetura a copiar.
```
...
O `NewScripts` contém uma versão funcionalmente rica do sistema Base 2.0, cobrindo lifecycle, rota, atividade, participantes, conteúdo local, materialização runtime, surfaces, input, save, pause, camera, audio, pooling, actors, projectiles, attributes, damage, diagnostics e QA.

O valor principal está nos **conceitos**, não no shape direto dos arquivos:

- lifecycle explícito por escopos;
- route como troca de contexto operacional;
- activity como unidade jogável/contextual;
- local como conjunto de objetos authored que contribuem capabilities;
- runtime spawned como materialização dinâmica com owner/lifetime;
- surface como contrato de espaço de apresentação;
- subsistemas como consumidores do lifecycle, não como centro da arquitetura;
- diagnostics/facts/readiness/fail-fast como instrumentos de previsibilidade.

O risco principal é que esses conceitos estão misturados em pipelines grandes, service locator, IDs textuais, assets “deus”, adapters que resolvem dependências diretamente, scene scans repetidos, runtime refs com validade implícita e subsistemas concretos dentro do fluxo core.

Conclusão arquitetural:

```text
Preservar: capacidades, contratos, boundaries e decisões funcionais.
Redesenhar: composition, identity, materialization, contribution discovery, ownership e release.
Descartar como API pública: pipelines Base 2.0, DependencyManager como locator, IDs por concatenação textual e subsistemas concretos no core.
```

---

## 2. Princípios extraídos do NewScripts

### 2.1. Princípios que devem sobreviver

| Princípio | Valor | Reinterpretação no framework novo |
|---|---|---|
| Escopos explícitos | Evita lifecycle ambíguo | `Session -> Route -> Activity -> Local`, com `RuntimeSpawned` e `Surface` como dimensões transversais. |
| Fail-fast | Evita fallback silencioso | Config/authoring obrigatório deve falhar cedo, com diagnóstico claro. |
| Authoring separado de runtime | Reduz acoplamento | Assets declaram intenção; runtime cria plans/results/sets/handles. |
| Plan antes de side effect | Torna transições auditáveis | `RouteSceneCompositionPlan`, `ActivityContentPlan`, `RuntimeMaterializationRequest`. |
| Result/handle após materialização | Permite release seguro | `ContentSet`, `MaterializationResult`, `RuntimeContentHandle`. |
| Contribution discovery | Permite extensibilidade local | `LocalContributionSet`, `RouteContributionSet`, `ActivityContributionSet`. |
| Readiness antes de reveal | Evita abrir gameplay incompleto | Readiness deve ser estado/resultado, não log. |
| Snapshot/restore por capability | Suporta save sem monolito | Payload versionado por capability, com identity typed. |
| Surface como contrato | Isola lugar de apresentação do consumidor | Pause/camera/UI/presentation consomem surface; não definem surface. |
| Diagnostics/facts | Permite QA e smoke dirigidos | Facts tipados mínimos; logs humanos não viram contrato funcional. |

### 2.2. Princípios que devem ser rejeitados

| Padrão rejeitado | Motivo |
|---|---|
| `DependencyManager.Provider` como API pública | Esconde dependência, enfraquece DIP e dificulta validação por escopo. |
| Pipeline monolítica como owner de subsistemas | Qualquer mudança em camera/audio/save/input altera lifecycle central. |
| IDs por string concatenada | Mistura domínios, impede validação forte e facilita stale/foreign matches. |
| Path/nome de GameObject como chave funcional | Quebra com rename/hierarchy/prefab changes. |
| Scene scan duplicado como resolução normal | Custo, ambiguidade e stale references. |
| Fallback silencioso | Mascaramento de authoring quebrado. |
| Logs como fonte de verdade | Confunde observabilidade com contrato. |
| Assets com campos de todos os subsistemas | Authoring vira god asset e captura ownership. |
| Runtime roots criados ad hoc por adapter | Duplicidade, órfãos e cleanup incerto. |
| Concrete subsystems no core inicial | Camera, Audio, Save, Pause, Actor, Projectile etc. passam a ditar o framework. |

---

## 3. Taxonomia consolidada

### 3.1. Escopos principais

| Escopo | Definição extraída | Papel no framework futuro |
|---|---|---|
| Session | Boundary acima de Route e Activity; boot, composition, estado global, participação, serviços persistentes e diagnostics. | Criar host/contexto/estado raiz; não conter gameplay concreto. |
| Route | Troca de contexto operacional; scene composition, transition, route state, route surfaces e handoff para Activity. | Navegação e materialização de contexto; não carregar subsistemas diretamente. |
| Activity | Unidade jogável/contextual dentro de uma Route; content loading, entry, contributors, readiness, reset/release. | Lifecycle de gameplay e content set de atividade. |
| Local | Objetos authored em scene/prefab que expõem capabilities, endpoints, slots, anchors ou markers. | Contribution/discovery/inventory por escopo. |
| RuntimeSpawned | Objetos criados dinamicamente por instantiate, pool, materializer ou adapter. | Materialização dinâmica com identity, owner, handle e release policy. |
| Surface | Espaço/slot/root/anchor onde conteúdo é apresentado ou resolvido. | Contrato de apresentação consumido por pause, camera, UI, presentation etc. |
| CrossCutting | Camera, Audio, Input, Save, Pause, Actor, Projectile, Pooling, Logging, QA e outros consumidores. | Módulos consumidores; alguns viram infra, outros packages opcionais. |

### 3.2. Termos canônicos recomendados

| Termo | Uso recomendado |
|---|---|
| `ContentSet` | Conjunto de conteúdo materializado ou descoberto em um escopo. |
| `ContributionSet` | Conjunto de contributions/capabilities descobertas a partir de conteúdo. |
| `MaterializationPlan` | Plano antes de criar/carregar algo. |
| `MaterializationResult` | Resultado após criar/carregar algo. |
| `RuntimeContentHandle` | Handle de instância materializada dinamicamente. |
| `LocalContentIdentity` | Identidade local tipada; substitui `targetId` solto como chave universal. |
| `SurfaceIdentity` | Identidade tipada de surface. |
| `SurfaceSlot` / `SurfaceAnchor` | Pontos authored de binding/resolução. |
| `LifecycleConsumer` | Subsistema que consome eventos/contextos de lifecycle sem possuir o lifecycle core. |
| `FrameworkFact` | Fato estruturado para diagnostics/QA, distinto de log humano. |
...
`Session` emerge no `NewScripts` como o boundary global acima de Route e Activity. Ela compõe dependências, mantém runtime mode/config, prepara estado operacional, participa da criação de serviços persistentes e fornece infraestrutura para Route/Activity.

### Capacidades principais

| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Bootstrap/composition | Inicializa runtime global | Preservar como `SessionCompositionRoot`, sem locator público. |
| Runtime config | Configura runtime mode e serviços | Preservar fail-fast; redesenhar como contexto explícito. |
| Persistent objects/services | Câmera/audio/input/session services persistem acima de Route | Preservar conceito de `SessionContentSet`; adiar concrete subsystems. |
| Player participation | Estado de participantes entre Route/Activity | Preservar como modelo typed; não misturar com materialização. |
| Diagnostics/facts | Observabilidade da sessão | Preservar como diagnostics core mínimo. |

### Riscos

- service locator global;
- composer operacional grande;
- subsistemas persistentes misturados com lifecycle;
- identity textual;
- dependências implícitas.

### Extração para framework

```text
SessionRuntimeHost
SessionRuntimeState
SessionCompositionRoot
SessionContentSet
SessionRuntimeRoot
SessionDiagnostics
...
`Route` é o núcleo de troca de contexto operacional: resolve um route asset, constrói plan, fecha a rota anterior, materializa cenas, estabiliza state, prepara surfaces/contributions e pode acionar Activity.

### Capacidades principais

| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Route authoring | Declara rota/cenas/startup activity | Preservar, mas dividir profiles/contributions. |
| Route identity | Diferencia rota/operação/transição | Tipar IDs; não usar concatenação textual. |
| Scene composition | Load/unload/active scene | Preservar plan/result/ownership. |
| Previous route exit | Exit-before-enter | Preservar como `RouteExitPlan`. |
| Route snapshot/state | Próxima rota sabe o que liberar | Preservar como `RouteRuntimeState`. |
| Route → Activity handoff | Activity inicia após rota pronta | Preservar, reduzir payload. |
| Route surfaces/contributions | Pause/camera/audio/input/save entram por rota | Redesenhar como `RouteContributionSet`/`RouteSurfaceSet`. |

### Riscos

- `SessionOperationalPipeline` monolítica;
- `OperationalRouteAsset` como god asset;
- Route sabendo detalhes de Activity/actors/save/pause;
- service locator em UI/adapters;
- materialização sem handles fortes.
...
RouteContentSet
RouteContributionSet
RouteSurfaceSet
RouteExitPlan
RouteStartupActivityPolicy
```
...
`Activity` é uma unidade jogável/contextual dentro de uma Route. Recebe handoff, carrega/respeita content profile, descobre contributors locais, materializa/binda participantes/actors, decide readiness e executa reset/snapshot/restore/release.

### Capacidades principais

| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Activity authoring | Define unidade jogável | Preservar declarativo; reduzir fields cross-cutting. |
| Activity content profile | Cenas/conteúdo da Activity | Preservar como `ActivityContentPlan`/`ActivityContentSet`. |
| Entry pipeline | Entrada separada da sessão/rota | Preservar conceito, não pipeline grande. |
| Contributor discovery | Encontra objetos locais | Preservar; gerar handles diretos e IDs typed. |
| Capability inventory | Organiza endpoints/providers | Preservar como contribution/capability set. |
| Participant binding | Conecta participação a actors/input | Preservar boundary, redesenhar ownership. |
| Readiness | Só revelar gameplay quando pronto | Preservar como state/result. |
| Reset/snapshot/restore/release | Lifecycle local/activity | Preservar por capability e policy. |

### Riscos

- `SessionActivityPipeline` monolítica;
- `ActivityEntryPipeline` acumulando subsistemas;
- identity textual composta;
- paths como identity;
- requiredness espalhada;
- actors/input/camera/pause/save dentro do entry core.
...
`Local` representa objetos authored em cenas ou prefabs que expõem capabilities para Activity/Route/subsystems por markers, endpoints, providers, slots, anchors e surfaces.
...
| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Contributor marker | Authoring explícito de objeto relevante | Preservar como `LocalContentContributionMarker`. |
| Discovery por escopo | Busca limitada ao conteúdo carregado | Preservar com `LocalContributionDiscoveryScope`. |
| Capability inventory | Lista endpoints/providers vivos | Preservar com validade por lifecycle. |
| Requiredness | Falha required, skip optional | Centralizar policy. |
| Reset/snapshot/restore/release | Lifecycle por componente | Preservar contratos, tipar matching. |
| Exit freeze | Congela correlação para teardown | Preservar para evitar stale state. |
| Slots/anchors | Pontos authored para binding | Preservar typed references. |

### Riscos

- `targetId` como chave universal;
- `ownerId`/`capabilityId` por concatenação;
- transform/component path como identity funcional;
- scan duplicado;
- runtime refs vivas após unload;
- stage conhece endpoint específico;
- fallback local sem policy.
...
`RuntimeSpawned` cobre qualquer objeto criado dinamicamente em runtime: actors, player actors, presentations, projectiles, pause content, audio objects, camera rigs, pool instances e runtime roots.

### Capacidades principais

| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Runtime roots | Lugar para instâncias dinâmicas | Preservar por escopo, com registry claro. |
| Materialization request/result | Criação auditável | Preservar como contrato genérico. |
| Runtime identity | Distinguir instâncias | Tipar; não usar nome de GameObject. |
| Pool rent/spawn | Reuso eficiente | Infra técnica separada do gameplay. |
| Release/return | Cleanup controlado | Policy explícita por owner/scope. |
| Spawn origin/slot | Posicionamento authored | Integrar com Local/Surface typed. |

### Riscos

- roots criados diretamente por adapters;
- `GameObject.Find` como resolução funcional;
- nomes de GameObject como identity;
- materializer fazendo root + identity + setup + registry + cleanup;
- destroy espalhado;
- pooling acoplado a projectile;
- lifecycle implícito global/singleton/root.
...
PooledContentMaterializer
RuntimeSpawnedContentSet
RuntimeOwnership
RuntimeReleasePolicy
RuntimeReturnPolicy
RuntimeSpawnedContribution
```
...
`Surface` é o contrato de espaço onde conteúdo é apresentado/resolvido. Ela não é pause, camera, UI ou presentation; esses são consumidores de surface.
...
| Capacidade | Valor funcional | Decisão futura |
|---|---|---|
| Surface endpoint | Expõe espaço de apresentação | Preservar, reduzir responsabilidades. |
| Surface profile | Declara surface/roots/slots | Preservar declarativo e typed. |
| Slot/anchor/root authored | Binding estável para conteúdo | Preservar; typed references obrigatórias. |
| Duplicate detection | Evita ambiguidade | Preservar em validators. |
| Surface context | Transporta resolução Route/Activity | Preservar, reduzir payload. |
| Binding por slot | Conteúdo vai para local correto | Preservar com handle/result. |
| Overlay root separado de content root | Papéis mais claros | Preservar como modelagem geral. |

### Riscos

- surface acoplada a pause/camera/presentation;
- strings soltas para `surfaceId`, `slotId`, `anchorId`, `rootId`;
- consumer controlando lifetime da surface;
- endpoint fazendo materialização demais;
- cleanup dependente de scene unload;
- Route/Activity sabendo demais sobre conteúdo dentro da surface.
...
## 4.7. CrossCutting / Subsystem Consumers

### Definição

Subsistemas transversais são consumidores de lifecycle, content, contribution, runtime identity, surface e diagnostics. Eles são valiosos, mas não devem comandar o core.

### Classificação consolidada

| Subsistema | Papel | Core? | Decisão futura |
|---|---|---:|---|
| Typed IDs / References | Primitivo transversal | Sim | Core inicial obrigatório. |
| Diagnostics / Facts | Infra transversal | Sim, mínimo | Core mínimo; QA opcional. |
| Runtime config | Infra transversal | Sim, explícito | Sem registry global público. |
| Event/foundation events | Comunicação | Sim, limitado | Scoped events/facts. |
| Pooling | Infra técnica | Parcial/opcional | Package técnico, consumido por RuntimeSpawned. |
| Input | Consumer intermediário | Não | Adapter/consumer contract. |
| Snapshot/Save | Consumer intermediário | Parcial | Snapshot contract no core; backend fora. |
| Pause | Consumer intermediário | Não | Consome Surface/Input/Activity. |
| Camera | Consumer visual | Não | Package/adapter opcional. |
| Audio | Consumer audiovisual | Não | Port/adapter opcional. |
| Actor/Player | Consumer gameplay | Não | Boundary próprio; não core lifecycle. |
| Attributes/Damage/Impact | Gameplay capabilities | Não | Capability modules. |
| Projectile | Advanced consumer | Não | Depois de RuntimeSpawned/Pooling/Actor. |
| QA/Debug panels | Tooling | Não | Pacote/ferramenta opcional. |

### Regra arquitetural

```text
Subsistemas declaram requisitos e recebem contexto.
Subsistemas não descobrem o mundo sozinhos.
Subsistemas não possuem o lifecycle core.
```
...
## 5.1. Lifecycle e estado

| Capacidade | Escopo primário | Nível | Observação |
|---|---|---|---|
| Session lifecycle host | Session | Básico | Root runtime e composition global. |
| Route transition | Route | Básico | Troca de contexto operacional. |
| Activity entry/exit | Activity | Básico/Intermediário | Gameplay contextual dentro da Route. |
| Previous route exit | Route | Intermediário | Protege cleanup antes do novo enter. |
| Readiness | Activity/CrossCutting | Intermediário | Evita reveal antes de setup. |
| Reset by scope | Activity/Local | Intermediário | Intents e profiles devem sobreviver. |
| Snapshot/restore | Local/Save | Intermediário/Avançado | Precisa schema e identity typed. |
| Release/return | RuntimeSpawned/Local | Intermediário | Policy explícita. |

## 5.2. Conteúdo e materialização

| Capacidade | Escopo primário | Nível | Observação |
|---|---|---|---|
| Scene composition | Route | Básico | Load/unload/active scene. |
| Activity content profile | Activity | Básico/Intermediário | Cenas/conteúdo de Activity. |
| Content loaded set | Activity/Route | Básico | Base para discovery. |
| Local contribution discovery | Local | Básico | Marker -> report -> inventory. |
| Runtime roots | RuntimeSpawned | Básico | Por escopo, com ownership. |
| Prefab materialization | RuntimeSpawned | Intermediário | Request/result/handle. |
| Pool materialization | RuntimeSpawned/Pooling | Intermediário | Infra técnica. |
| Surface binding | Surface | Intermediário | Slot/anchor/root typed. |

## 5.3. Identidade e referências

| Família | Situação no NewScripts | Decisão futura |
|---|---|---|
| Session/Route/Activity IDs | Conceito forte, misturado com strings | Tipar e separar domínios. |
| PlayerSlot/Participant IDs | Útil para participation/input/actor | Preservar typed. |
| ActorId/ActorInstanceRuntimeId | Essencial para actors runtime | Preservar e formalizar. |
| Local `targetId` | Cola universal frágil | Substituir por `LocalContentIdentity`. |
| Surface/slot/anchor IDs | Úteis, hoje string/manual | Typed references. |
| Capability/permission/receiver IDs | Úteis, algumas partes tipadas | Consolidar policy. |
| Snapshot IDs/schema | Conceito forte | Schema/versioning explícito. |
| Operation IDs | Muitas vezes string composta | IDs estruturados. |

## 5.4. Diagnostics, QA e validação

| Capacidade | Valor | Decisão futura |
|---|---|---|
| Fact recorder | Trilha auditável | Preservar como facts tipados. |
| DebugUtility | Conveniência MVP | Não virar global invisível. |
| QA panels/buttons | Smoke manual | Tooling opcional. |
| Validators | Segurança de authoring | Manter editor-only/idempotente. |
| Logs estruturados | Diagnóstico | Não substituir contrato runtime. |

---

## 6. Preservar / Redesenhar / Descartar

## 6.1. Preservar

- Boundaries: Session, Route, Activity, Local.
- Materialization como plan/result/handle.
- Content sets e loaded sets.
- Contribution discovery e capability inventory.
- Route exit antes de route enter.
- Route → Activity handoff explícito.
- Readiness antes de reveal.
- Runtime roots por escopo.
- Surface como contrato de espaço.
- Slots/anchors authored.
- Required/optional com fail-fast em required.
- Reset/snapshot/restore/release por capability.
- Stale/foreign checks.
- Snapshot envelopes com owner/schema/payload.
- Ports/adapters entre lifecycle e subsistemas.
- Runtime config obrigatória.
- QA/smoke como ferramenta manual separada.

## 6.2. Redesenhar

- `DependencyManager` como locator público.
- `SessionOperationalPipeline` e `SessionActivityPipeline` como monolitos.
- `OperationalRouteAsset` e `ActivityAsset` com campos de subsistemas demais.
- `targetId`/`ownerId`/`capabilityId` por string.
- Scene scan duplicado como resolução principal.
- Runtime refs sem validade explícita por lifecycle.
- Requiredness espalhada.
- Activity/Route conhecendo endpoints concretos.
- Surface endpoint acumulando binding, materialization e lifecycle owner.
- Save payload detection por texto.
- Input action map por string sem boundary.
- Actor command enum hardcoded em contrato genérico.
- Pooling acoplado a projectile.
- Camera/audio/pause/save/input dentro do core lifecycle.

## 6.3. Descartar como API pública

- Pipelines Base 2.0 diretamente.
- `DependencyManager.Provider` em consumers públicos.
- IDs por concatenação textual.
- Transform path/GameObject name como chave funcional.
- Fallback silencioso.
- Logs como fonte de verdade.
- Concrete Cinemachine, Audio service, Save backend, PlayerInput, Projectile, Damage, QA panels no core inicial.
- Cleanup baseado só em scene unload.
- Materializers que fazem tudo: resolve, instantiate, parent, registry, setup, cleanup e diagnostics sem boundaries.

---

## 7. Dependências e ordem recomendada

## 7.1. Ordem conceitual para framework limpo

```text
1. Identity typed core
2. Diagnostics/facts mínimos
3. Session host/state/composition explícita
4. Content/materialization primitives
5. Route mínimo: asset, transition, scene composition, runtime state
6. Activity mínimo: entry context, content set, readiness
7. Local: identity, contribution marker, discovery, contribution set
8. RuntimeSpawned: roots, request/result/handle, release policy
9. Surface: identity, endpoint, slot, anchor, binding result
10. Capability inventory genérico
11. Reset/release local/activity
12. Snapshot/save contracts
13. Input consumer
14. Pause/surface consumer
15. Camera/audio consumers
16. Actor materialization
17. Pooling package integration
18. Projectile/damage/attributes/impact
19. QA/debug tooling
```

## 7.2. Ciclo recomendado de evolução

A evolução não deve aprofundar um único escopo até o fim. Deve repetir ciclos:

```text
Ciclo 1 — Base mínima
Session → Route → Activity → Local

Ciclo 2 — Content e contribution
SessionContentSet → RouteContentSet → ActivityContentSet → LocalContributionSet

Ciclo 3 — Materialization e release
Runtime roots → Route materialization → Activity materialization → Local lifecycle

Ciclo 4 — Surface e spawned
Surface contracts → RuntimeSpawned handles → slots/anchors → release policy

Ciclo 5 — Consumers intermediários
Input → Snapshot/Save contract → Pause

Ciclo 6 — Consumers avançados
Camera → Audio → Actor → Pooling → Projectile/Damage/Attributes
```

---

## 8. Backlog consolidado de ADRs

## 8.1. ADRs core

- ADR: definição de escopos `Session`, `Route`, `Activity`, `Local`.
- ADR: `RuntimeSpawned` como dimensão transversal, não escopo concorrente.
- ADR: `Surface` como contrato de espaço, não subsistema.
- ADR: política de typed identity e typed references.
- ADR: política para diagnóstico versus chave funcional.
- ADR: composição explícita e proibição de service locator público.
- ADR: lifecycle consumer contract model.
- ADR: content/materialization plan/result/handle.
- ADR: content set e contribution set.
- ADR: runtime root ownership por escopo.

## 8.2. ADRs Route/Activity/Local

- ADR: Route asset mínimo e route contribution profiles.
- ADR: scene composition e ownership de cenas.
- ADR: active scene policy.
- ADR: route exit antes de route enter.
- ADR: handoff Route → Activity.
- ADR: Activity content profile e loaded set.
- ADR: Activity readiness.
- ADR: Local contribution discovery.
- ADR: Local identity e replacement de `targetId`.
- ADR: required/optional local.
- ADR: runtime references e validade por lifecycle.
- ADR: reset/snapshot/restore/release local.

## 8.3. ADRs consumers/infra

- ADR: snapshot/save payload model e schema versionado.
- ADR: surface consumer model.
- ADR: pause como consumer de surface/input/activity.
- ADR: input mode ownership.
- ADR: camera como presentation consumer.
- ADR: audio como lifecycle/spawned consumer.
- ADR: actor runtime boundary.
- ADR: pooling ownership e package boundary.
- ADR: projectile como advanced consumer.
- ADR: diagnostics/facts/logging boundary.
- ADR: QA/debug tooling fora do runtime core.

---

## 9. Perguntas abertas consolidadas

- Qual é o menor conjunto de lifecycle events que consumers precisam?
- `CapabilitySnapshot` deve ser core ou módulo intermediário?
- `Pooling` fica como package técnico independente ou infra interna?
- `Input Mode` pertence ao framework ou a adapter Unity/InputSystem?
- `Pause Surface` é caso específico ou primeira forma de `SurfaceConsumer`?
- `Actor` deve existir no framework core ou apenas como consumer sample?
...
- Como invalidar runtime refs após unload?
- Route-local e Activity-local usam o mesmo modelo de contribution?
- Anchors/slots são genéricos de Surface ou APIs especializadas?
- Route deve iniciar Activity diretamente ou emitir `RouteReady`?
- Active scene é sempre obrigatório?
- O framework suporta rota ativa única ou stack de rotas?
- Diagnostics/facts são API pública ou tooling interno?

---

## 10. Síntese final para orientar a próxima fase

A auditoria isolada do `NewScripts` fecha com esta leitura:

```text
O sistema antigo prova que o modelo Session → Route → Activity → Local é funcional.
O sistema antigo também prova que, se subsistemas concretos entram cedo demais, eles capturam o lifecycle.
```

Portanto, a próxima fase deve ser a auditoria isolada do package atual `com.immersive.framework`, procurando especificamente:

1. Quais desses conceitos já existem.
2. Quais existem com nome errado ou shape prematuro.
3. Quais estão faltando.
4. Quais foram implementados cedo demais.
5. Quais devem ser removidos, congelados ou reclassificados.

A comparação só deve acontecer depois da auditoria do package atual. Até lá, este documento é a fonte consolidada do `NewScripts`.

---

# Parte II — Registro integral das auditorias consolidadas

A partir daqui, o conteúdo preserva o registro detalhado acumulado das auditorias. Esta parte é mantida para não perder achados, evidências, tabelas e nuances que não cabem na síntese acima.
