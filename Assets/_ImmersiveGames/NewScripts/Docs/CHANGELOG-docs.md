...
* Updated: `WORLD_LIFECYCLE.md` e/ou `WORLDLIFECYCLE_RESET_STATUS.md` com fechamento do hardening do Item 2:

    * `contextSignature` padronizado via `SceneTransitionSignatureUtil.Compute(context)` (compatível com `context.ToString()` hoje) e consistente entre publisher e gate.
    * `reason` padronizado via `WorldLifecycleResetReason.*` (ex.: `ScenesReady/<ActiveScene>` e `Skipped_StartupOrFrontend:profile=...;scene=...`).
* Updated: `WORLD_LIFECYCLE.md` (ou seção equivalente de runtime) com registro de ownership e limpeza:

    * responsabilidades globais: `WorldLifecycleRuntimeCoordinator` (gatilho + publish), `WorldLifecycleResetCompletionGate` (await + cache defensivo/timeout);
    * responsabilidades por cena: `SceneServiceRegistry`/`NewSceneBootstrapper` + limpeza em unload via `SceneServiceCleaner`.
