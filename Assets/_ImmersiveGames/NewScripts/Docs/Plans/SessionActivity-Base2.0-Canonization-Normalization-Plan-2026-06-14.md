...
**Status:** Phase 0 + Phase 1 (B0/B1 + múltiplas ondas de B2 + batches complexos G1/G2 de ObjectSetup) **concluídos com sucesso**. Compilação/smoke limpos validados nos checkpoints manuais mais recentes. 

**Próxima prioridade:** Concluir os resíduos explícitos de B2 sem reabrir ObjectSetup: ParticipantBinding completo / ExitActorTeardown se a auditoria justificar; depois Fase 2 (Host + Route-Exit). Content unload/release permanece sem ação imediata enquanto não houver regressão.
...
- Bridge proliferation: broad `IActivityEntryRuntimeBridge` aggregate still exists in residual surfaces; `BindEntryPipeline` seam was removed in SA-19B1.
- `SessionActivityPipeline` implements too many bridges and acts as god object.
- `SessionActivityHost` is not yet thin.
- Generic coordinators/runners: `ActivityCapabilityInventoryCoordinator` **fechado/removido do caminho ativo em SA-19B2-G1**; `PendingOperationRunner` ainda pendente.
...
- Consolidated status report precisa ser sincronizado no repositório; cópia local atualizada: `SessionActivity-2.0-Current-Status.md`.
...
**SA-19B2 — Stage-by-Stage Bridge Narrowing (em andamento / major waves + ObjectSetup complex batch concluídas)**
- **Status:** Múltiplas ondas concluídas com ~10+ stages migrados + `SA-19B2-G1/G2` fechados com PASS. Compilação/smoke manuais validados nos checkpoints recentes.
- Prioridade atual: ObjectSetup composite fechado. Restam apenas resíduos explícitos: ParticipantBinding completo e ExitActorTeardown se a auditoria justificar; Content unload/release permanece sem ação imediata por ter sido classificado como owner correto / baixo retorno.
...
- **Status:** CLOSED by `SA-19B2-G1`.
- Chosen shape: option B — eliminate it from the active path.
- `ActivityCapabilityInventoryCoordinator` was removed from active Entry Setup and replaced by the explicit preview port `IActivityCapabilityInventoryPreviewSource` / `ActivityCapabilityInventoryPreviewSource`.
- Canonical justification: preview is a technical inventory read/build boundary for Entry Setup, not a coordinator/lifecycle owner.
- **Acceptance:** No active coordinator path; no new god object; inventory remains technical index.
...
- SA-19B2-COMPLEX-AUDIT + G1/G2: ObjectSetup complexo agrupado e fechado com PASS; coordinator genérico removido; composite dividido em stages concretos

**Próxima prioridade imediata (alto leverage):**
- Concluir apenas os resíduos explícitos de SA-19B2: ParticipantBinding completo e/ou ExitActorTeardown, sempre com auditoria curta antes. Não reabrir ObjectSetup composite; Content unload/release fica em no-action enquanto não houver regressão concreta.
...
- Fases 0 e 1 (B0 + B1 + várias ondas B2 + G1/G2 ObjectSetup) **concluídas**.
- Compilação validada com sucesso (fulllog.txt).
- Redução concreta da proliferação de bridges aggregate.
- Próximo: auditar os resíduos B2 restantes sem reabrir ObjectSetup; depois Fase 2 (Host).
