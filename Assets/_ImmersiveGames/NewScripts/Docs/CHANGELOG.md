...
- **Compat residual removida:** o soft reset real de players continua funcionando, agora via `ByActorKind(Player)`, e `PlayerActorAdapter` passa a expor `IActorKindProvider` para participar do contrato canonico.
...
- **Excecoes registradas com precisao:** permanece apenas o pequeno residuo editor/serializado em `GameNavigationIntentCatalogAsset` fora do fechamento principal.
