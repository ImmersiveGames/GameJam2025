...
## Atualização (2025-12-24) — checklist baseado em log real

### Checagens adicionais (Gate/Readiness)

- [ ] Ao iniciar baseline (Hard Reset / Soft Reset), `GameReadinessService` publica snapshot com `gameplayReady=False` (reason `baseline_started/...`).
- [ ] Durante o reset, existe token de gate:
    - [ ] `WorldLifecycle.WorldReset` no hard reset
    - [ ] `flow.soft_reset` no soft reset
- [ ] Após reset, `SimulationGateService` libera o token e `Active=0` (gate aberto).
- [ ] Após baseline concluído, snapshot com `gameplayReady=True` e `reason` coerente (`baseline_completed/...`).

### Checagem de coerência com GameLoop (quando aplicável)
- [ ] Mesmo com `gameplayReady=True`, ações podem permanecer bloqueadas por **NotPlaying** se `serviceState=Menu` (isso é esperado).
- [ ] Ao disparar `GameStartRequestedEvent`, o fluxo deve passar por `SceneTransitionStarted` → `ScenesReady` → `Completed` e apenas então liberar input.
