...
## Addendum — SA-5A0 + SA-5A1 aplicados neste pacote
...
- Não continuar cortes por `PlayerActorReadiness` ou `NonPlayerActorDiscovery` como shape final.
- Próximo corte de actors deve ser `ActorDiscovery` genérico ou auditoria equivalente.
- `PlayerActorId`, `PlayerSlotId`, `ActorId`, `ActorInstanceRuntimeId`, `ReceiverId` e futuro `PermissionTargetId` não são equivalentes.
- `TargetActorId` é o alvo de permission actor-scoped; `ReceiverId` é apenas receiver técnico.
- `CameraBindingRequirement.TargetId` não deve ser comparado contra `PlayerSlotId`.

Status: código/documentação preparados; aceite depende de smoke/log manual.
