...
- `PoolDefinitionAsset.prewarm` agora e aplicado por `PoolService.EnsureRegistered` no primeiro registro do pool.
- Consumers de gameplay nao devem chamar `Prewarm` para aplicar policy de authoring.
- `Prewarm(definition)` permanece disponivel para QA/manutencao explicita.
- O caminho de projectile deve depender do pool canonico ja registrado/aquecido, sem lifecycle paralelo.
