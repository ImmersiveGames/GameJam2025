...
| `PlayerActorResetEndpointResolver.ResolveOrFail(...)` | Removeu rejeição antecipada por `actor.PlayerActorId.IsValid == false`. | Resolver técnico. | Lookup continua por `TryGetActiveHandleByActorInstance(...)` / `TryGetRouteScopedHandleByActorInstance(...)`. |
