...
## ACT-PROJ-2B — Projectile spawn adapter uses canonical pool service — APPLIED

### Status

```text
APPLIED / FirePrimary delegates to spawn adapter; adapter rents RuntimeSpawnedActor from canonical IPoolService; sem ObjectEmission e sem manager novo.
```

### Decisão

```text
ActorProjectileFireEndpoint continua sendo o sink canônico de FirePrimary, mas não é owner de pool, spawn lifecycle ou reset.
ActorProjectileSpawnAdapterBoundary passa a executar apenas a side-effect técnica de Rent via IPoolService canônico.
ActorSpawnabilityProfileAsset agora exige PoolDefinitionAsset tipado para RuntimeSpawned/ReturnToOriginPool executável.
PoolDefinitionAsset é a identidade técnica do pool; strings de pool ficam apenas como labels/IDs diagnósticos.
O prefab spawnado é RuntimeSpawnedActor, mantendo o princípio de que spawnables de gameplay são Actors.
```

### Arquivos introduzidos/alterados

```text
Actors/Runtime/RuntimeSpawnedActor.cs
Actors/Foundation/ActorIdentityContracts.cs
Actors/Foundation/ActorModelContracts.cs
Actors/Semantic/Participation/ActorSpawnabilityProfileAsset.cs
Actors/Projectile/Contracts/ActorProjectileContracts.cs
Actors/Projectile/Contracts/ActorProjectileSpawnAdapterContracts.cs
Actors/Projectile/Runtime/ActorProjectileFireEndpoint.cs
Actors/Projectile/Runtime/ActorProjectileSpawnAdapterBoundary.cs
Resources/Actors/ProjectileActor_PlayerPrimary.prefab
Resources/Actors/PoolDefinition_PlayerPrimaryProjectile.asset
Resources/Actors/ActorSpawnabilityProfile_PlayerPrimaryProjectile.asset
```
...
```text
Não houve ObjectEmission.
Não houve ProjectileManager, SpawnableManager ou manager novo.
Não houve collision, damage, VFX, audio, motion runtime, lifetime executor, reset/save ou alteração de pipeline.
Pool continua adapter técnico; não decide policy/lifecycle.
RuntimeSpawnedActor recebe identidade runtime no rent; reset/return ao pool fica para corte posterior.
```
...
ActorCommandSinkBound commandId='FirePrimary' sink='ActorProjectileFireEndpoint' state='Executable'.
ActorProjectileFireCommandBuilt reason='spawn_adapter_command_built'.
ActorProjectileSpawnPoolRentRequested poolDefinition='PoolDefinition_PlayerPrimaryProjectile'.
ActorProjectileSpawnedFromPool spawnExecuted='True' poolCalled='True'.
ActorProjectileFireSpawnAdapterCompleted adapterResult='Spawned' spawnExecuted='True' poolCalled='True'.
...
Não deve aparecer ObjectEmission no gate/surface ativo.
Não deve aparecer ObjectEmissionSpawned.
Deve aparecer spawnExecuted='True' e poolCalled='True' somente no ActorProjectileSpawnAdapterBoundary/ActorProjectileFireEndpoint.
```

## ACT-PROJ-2C — ObjectEmission legacy runtime deletion — APPLIED

### Status

```text
APPLIED / ObjectEmission removido do composition graph e marcado para deleção física.
```

### Decisão

```text
Projectile spawn via ActorProjectileFireEndpoint + ActorProjectileSpawnAdapterBoundary substitui o antigo caminho ativo de ObjectEmission para FirePrimary.
ObjectEmission não é mais bridge transitória necessária no runtime ativo.
GlobalCompositionRoot não compõe mais ObjectEmissionRuntimeComposer nem ObjectEmissionPoolRuntimeBridge.
Assets e scripts ObjectEmission são removidos por DELETE_FILES.txt para evitar ambiguidade de configuração.
Histórico de ADR permanece apenas como evidência, não como contrato ativo.
```
...
### Arquivos removidos por manifesto

```text
Actors/ObjectEmission/**
Resources/Actors/ActorObjectEmissionProfile.asset
Resources/Actors/ObjectEmission_Generic.prefab
Resources/Actors/PoolDefinitionObjectEmission.asset
Actors/Projectile/Authoring/ActorProjectileFireAuthoringMarker.cs
```
...
```text
Não remove Pooling canônico.
Não remove Projectile runtime novo.
Não altera ActorProjectileFireEndpoint.
Não altera ActorProjectileSpawnAdapterBoundary.
Não altera SessionActivityPipeline ou ActivityEntryPipeline.
Não cria fallback, alias, compat ou trilho paralelo.
```
...
```text
Compile sem error CS.
Smoke sem ObjectEmissionRuntimeComposer.
Smoke sem ObjectEmissionPoolRuntimeBridge.
Smoke sem ObjectEmissionPoolServiceAttached.
Smoke sem ObjectEmissionSpawned.
Smoke com ActorProjectileSpawnedFromPool preservado.
Smoke com RestartCurrentActivity, Activity01ToActivity02 e RouteExitBackToMenu PASS.
```

### Observação pós-smoke ACT-PROJ-2B

```text
O smoke de 2B confirmou spawn via pool canônico, mas também revelou que FirePrimary ainda executa em DeactivationWindowReady enquanto Move está RejectedInactive.
Isso não é legado de ObjectEmission; é ausência de policy/gate para comandos executáveis de gameplay no novo ProjectileFire.
Deve virar corte separado depois da deleção do legado: ACT-PROJ-2D — Projectile fire gameplay-state gate.
```


## ACT-PROJ-2D — Projectile fire gameplay-state gate alignment — CLOSED
...
```text
ProjectileFire passa a respeitar o mesmo ActivityGameplayControl gate usado por Movement.
ActivityCapabilityPermissionScanner registra movement.receiver e projectile_fire.receiver quando o Actor expõe os dois endpoints.
ActivityCapabilityPermissionRuntime permanece owner do estado Allowed/Blocked/Unbound.
ActorProjectileFireEndpoint mantém apenas estado local enabled/disabled e rejeita comando quando inativo.
ActorProjectileSpawnAdapterBoundary continua sendo o único ponto técnico de pool/rent.
```
...
Actors/Projectile/Contracts/ActorProjectileEndpointContracts.cs
Actors/Projectile/Runtime/ActorProjectileFireEndpoint.cs
Actors/Projectile/Runtime/ActorProjectileFirePermissionReceiverProvider.cs
SessionActivity/Capabilities/Inventory/ActivityCapabilityPermissionScanner.cs
```
...
```text
Não cria gate novo.
Não cria permission runtime paralelo.
Não move lifecycle para endpoint.
Não altera pool/spawn adapter.
Não reintroduz ObjectEmission.
Não altera SessionActivityPipeline ou ActivityEntryPipeline.
Não resolve ainda a frente ampla de IDs/string; isso fica para corte futuro próprio.
```
...
```text
Compile sem error CS.
Gate deve registrar movement.receiver e projectile_fire.receiver.
Durante ActivationWindowReady/DeactivationWindowReady, FirePrimary deve retornar RejectedInactive com dispatchReason='projectile_fire_endpoint_inactive'.
Durante ActivityRunning, FirePrimary deve manter ActorProjectileSpawnedFromPool com spawnExecuted='True' e poolCalled='True'.
Move deve permanecer RejectedInactive quando o movement endpoint estiver inativo.
Sem ObjectEmissionRuntimeComposer, ObjectEmissionPoolRuntimeBridge, ObjectEmissionPoolServiceAttached ou ObjectEmissionSpawned.
RestartCurrentActivity, Activity01ToActivity02 e RouteExitBackToMenu devem continuar PASS.
```

## ACT-PROJ-2E — Projectile MVP closure documentation — CLOSED

### Status

```text
CLOSED / DOCUMENTATION ONLY / fechamento do MVP de ProjectileFire.
```

### Decisão de fechamento

```text
O MVP de ProjectileFire fica congelado como funcional e arquiteturalmente aceito no escopo atual.
FirePrimary usa ActorCommandHub.
ProjectileFire é Actor capability local.
ActorProjectileFireEndpoint é o sink canônico de FirePrimary.
ActorProjectileSpawnAdapterBoundary é o único adapter técnico de pool/rent.
Pooling canônico permanece via IPoolService.
ActivityGameplayControl é o gate comum para Movement e ProjectileFire.
ObjectEmission saiu do caminho ativo e não deve ser reintroduzido como bridge de FirePrimary.
```

### Caminho runtime congelado

```text
PlayerInput
-> PlayerActorCommandInputHub
-> ActorCommandEnvelope commandId='FirePrimary'
-> ActorProjectileFireEndpoint
-> ActorProjectileFireCommand
-> ActorProjectileSpawnAdapterBoundary
-> IPoolService
-> RuntimeSpawnedActor / ProjectileActor_PlayerPrimary
```

### Owners finais do MVP

| Responsabilidade | Owner aceito |
|---|---|
| Resolver input/action local do Actor | `PlayerActorCommandInputHub` |
| Binding de command para capability | `ActorCommandBindingAdapter` / `ActivityEntryPipeline` |
| Capability local de fire | `ActorProjectileFireEndpoint` |
| Side-effect técnico de spawn/rent | `ActorProjectileSpawnAdapterBoundary` |
| Pool técnico | `IPoolService` canônico |
| Permission state `Allowed/Blocked/Unbound` | `ActivityCapabilityPermissionRuntime` |
| Registro de receivers no gate | `ActivityGateBindingStage` + `ActivityCapabilityPermissionScanner` |
| Macro lifecycle da Activity | `SessionActivityPipeline` |
| Ordem/setup/binding da entry | `ActivityEntryPipeline` |

### Evidência aceita do smoke ACT-PROJ-2D

```text
Gate registra movement.receiver e projectile_fire.receiver.
ActivityGateBindingCompleted receivers='2'.
FirePrimary em ActivationWindowReady/DeactivationWindowReady retorna RejectedInactive.
dispatchReason='projectile_fire_endpoint_inactive'.
FirePrimary em ActivityRunning executa spawn via pool canônico.
ActorProjectileSpawnedFromPool aparece com spawnExecuted='True' e poolCalled='True'.
RestartCurrentActivity passa.
Activity01ToActivity02 passa.
RouteExitBackToMenu passa.
Não aparece ObjectEmissionRuntimeComposer, ObjectEmissionPoolRuntimeBridge, ObjectEmissionPoolServiceAttached, ObjectEmissionSpawned, ActorObjectEmitterEndpoint ou object_emission.receiver.
```

### Fronteiras preservadas

```text
ActorProjectileFireEndpoint não decide lifecycle global.
ActorProjectileFireEndpoint não chama IPoolService diretamente.
ActorProjectileSpawnAdapterBoundary não decide permission, gate ou lifecycle.
ActivityCapabilityPermissionRuntime não executa spawn.
ActivityGateBindingStage não conhece pool, projectile prefab, damage, VFX ou audio.
SessionActivityPipeline não recebeu lógica concreta de projectile.
ActivityEntryPipeline apenas orquestra setup/binding; não executa projectile.
```

### Legado removido / proibido

```text
ObjectEmission não é owner ativo de FirePrimary.
ActorObjectEmitterEndpoint não deve voltar ao ActorCapabilitySurface.
ObjectEmission receiver não deve voltar ao ActivityGameplayControl.
ObjectEmissionRuntimeComposer não deve voltar ao composition graph para este MVP.
Não criar PlayerShootController, ProjectileManager, SpawnableManager ou pool paralelo.
Não usar UnityEvent como bridge de command/capability.
Não criar reader novo por capability.
```

### Débitos explicitamente fora do MVP

```text
ACT-PROJ-IDREF — typed reference hygiene para command/projectile/pool.
Alinhar Movement e Fire em modelo transversal comum de command/capability/gate sem relações por string.
Trocar relações authoring/runtime ainda textuais por referências tipadas quando existir asset/contrato adequado.
Projectile lifetime executor e retorno automático ao pool.
Projectile motion.
Collision/impact.
Damage capability.
VFX.
Audio/SFX via AudioRuntime.
Save/snapshot de runtime-spawned Actors apenas se houver caso real.
Determinismo/seed de spread.
Profile/presentation avançada de projectile.
```

### Regra para próximos cortes

```text
Próximos cortes podem avançar para lifetime/motion/collision/damage/audio ou IDREF hygiene.
Nenhum desses cortes pode reabrir ObjectEmission como owner de FirePrimary.
Nenhum corte deve transformar ActorProjectileFireEndpoint em manager de pool/lifetime/damage/audio.
Qualquer capability nova deve passar por endpoint/capability local, policy/gate explícito e adapter técnico separado.
```

### Critério de regressão futura

Um corte futuro que tocar ProjectileFire deve ser considerado regressão se aparecer qualquer um destes sinais:

```text
ObjectEmissionRuntimeComposer
ObjectEmissionSpawned
ActorObjectEmitterEndpoint
object_emission.receiver
FirePrimary spawnando quando ActivityGameplayControl está Blocked ou Unbound
ActorProjectileFireEndpoint chamando pool diretamente
SessionActivityPipeline executando spawn/projectile side-effect
ActivityGateBindingCompleted receivers menor que 2 quando Movement e ProjectileFire estão presentes
...
O MVP de ProjectileFire fechou funcionalmente, mas a revisão pós-MVP identificou dois problemas arquiteturais no `PlayerActorCommandInputHub`:

```text
1. `ActorCommandSourceKind` era falso genérico no path atual, porque o único producer real é `PlayerActorCommandInputHub`.
2. `ActorCommandInputBinding` ainda carregava `ActionMapName` e `ActionName`, duplicando ownership com `InputModes`.
```

### Decisão

```text
InputModes é owner de modo e ActionMap ativo.
PlayerActorCommandInputHub é apenas adapter local de PlayerInput -> ActorCommandEnvelope.
ActorCommandBinding não decide ActionMap.
ActorCommandBinding usa InputActionReference tipado.
SourceKind sai do contrato ativo de ActorCommand.
SourceIdentity permanece apenas como metadata observacional do producer local.
```

### Mudança estrutural

```text
ActorCommandSourceKind removido do command path.
ActorCommandInputBinding.SourceKind removido.
ActorCommandInputBinding.ActionMapName removido.
ActorCommandInputBinding.ActionName removido.
ActorCommandInputBinding.ActionReference deixou de ser hidden e virou referência obrigatória.
IActorCommandSourceHub.HasBinding agora valida apenas commandId + triggerKind.
ActorCommandEnvelope não carrega SourceKind.
PlayerActorCommandInputHub resolve a action tipada contra o PlayerInput.actions ativo por InputAction.id, não por nome de map/action.
PlayerInputBindingAdapter não altera defaultActionMap nem chama SwitchCurrentActionMap.
PlayerMovementController não rejeita command por SourceKind.
```

### Ownership final deste corte

```text
InputModes:
  decide InputMode e ActionMap ativo.

PlayerInputBindingAdapter:
  garante PlayerInput/canonical actions/slot binding.
  não troca ActionMap.

PlayerActorCommandInputHub:
  resolve InputActionReference tipado dentro do PlayerInput.actions ativo.
  traduz Move/FirePrimary para ActorCommandEnvelope.
  não decide mapa e não roteia por SourceKind.

ActorCommandHub/Sinks:
  roteiam por ActorCommandId/BindingId.

ActivityCapabilityPermissionRuntime:
  decide Allowed/Blocked/Unbound para Movement e ProjectileFire.
```

### Configuração obrigatória após o corte

Como `ActionMapName`/`ActionName` foram removidos, o prefab do Player deve ter referências tipadas no `PlayerActorCommandInputHub`:

```text
binding 'move' -> ActionReference = PlayerInputActions/Player/Move
binding 'fire_primary' -> ActionReference = PlayerInputActions/Player/Fire
```

Ausência dessas referências deve falhar explicitamente no prepare do hub; não há fallback textual por map/action.

### Critério de aceite

```text
sem error CS
ActorCommandBindingResolved não deve logar sourceKind
ActorCommandBindingResolved deve logar observedActionMap/observedAction apenas como observação
Move continua funcional
FirePrimary continua respeitando ActivityGameplayControl
FirePrimary spawna via ActorProjectileSpawnAdapterBoundary durante ActivityRunning
FirePrimary é RejectedInactive fora de ActivityRunning
InputModes continua emitindo InputModeApplied
PlayerInputBindingAdapter não emite SwitchCurrentActionMap próprio
ObjectEmission não reaparece
RestartCurrentActivity PASS
Activity01ToActivity02 PASS
RouteExitBackToMenu PASS
```
...
O MVP de `ProjectileFire` funcionou, mas o prefab do Player ainda carregava três peças para tiro:
...
`PlayerActorCommandInputHub` é aceitável porque é o adapter local `PlayerInput -> ActorCommandEnvelope`.
`ActorProjectileFireEndpoint` é aceitável porque é a capability local do Actor.
`ActorProjectileSpawnAdapterBoundary` como componente serializado no Player era configuração técnica demais no prefab e foi classificado como bridge transitória.

### Decisão

```text
Endpoint continua no Actor.
Spawn adapter sai do prefab.
Spawn adapter vira classe runtime não-MonoBehaviour resolvida/configurada no binding.
ActivityEntry/ActorCommandBindingAdapter passa a configurar o endpoint com IActorProjectileSpawnAdapter.
...
```text
ActorProjectileFireEndpoint não serializa mais spawnAdapter.
IActorProjectileFireEndpoint expõe ConfigureSpawnAdapter(IActorProjectileSpawnAdapter, source, reason).
Criado PooledActorProjectileSpawnAdapter como classe runtime não-MonoBehaviour.
ActorCommandBindingAdapter cria/configura PooledActorProjectileSpawnAdapter quando o endpoint está Prepared.
ActorCommandBindingAdapter continua fazendo BindCommandSink(FirePrimary, endpoint).
PlayerActor_v0.prefab mantém Hub + Endpoint e remove o componente ActorProjectileSpawnAdapterBoundary.
ActorProjectileSpawnAdapterBoundary deve ser removido via DELETE_FILES.txt.
```
...
```text
PlayerActorCommandInputHub:
  traduz input local para ActorCommandEnvelope.

ActorProjectileFireEndpoint:
  capability local do Actor.
  monta ActorProjectileFireCommand.
  não resolve IPoolService.

PooledActorProjectileSpawnAdapter:
  adapter técnico runtime.
  usa IPoolService canônico.
  executa Rent e bind de RuntimeSpawnedActor.

ActorCommandBindingAdapter:
  stage adapter de binding.
  conecta endpoint ao spawn adapter técnico.
  binda FirePrimary ao endpoint.
```
...
```text
sem error CS
PlayerActor_v0.prefab não contém ActorProjectileSpawnAdapterBoundary
ActorProjectileSpawnAdapterBoundary.cs removido
ActorProjectileFireEndpoint não tem campo serializado spawnAdapter
ActorProjectileSpawnAdapterConfigured aparece antes de ActorCommandSinkBound
ActorCommandSinkBound commandId='FirePrimary' state='Executable'
ActorProjectileSpawnedFromPool aparece durante ActivityRunning
FirePrimary segue RejectedInactive fora de ActivityRunning
ObjectEmission não reaparece
RestartCurrentActivity PASS
Activity01ToActivity02 PASS
RouteExitBackToMenu PASS
