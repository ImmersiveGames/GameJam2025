...
Objetivo: mover `ActorPresentation` setup para stage real de entry, sem transformar `ActivityEntryPipeline.cs` em mini-pipeline. A execução concreta fica em `ActivityEntryMovementBindingStage`. O stage legado `PlayerMovementBindingStage` foi removido para não manter trilho paralelo ativo.
...
Release -> ActivityObjectReleaseStage
