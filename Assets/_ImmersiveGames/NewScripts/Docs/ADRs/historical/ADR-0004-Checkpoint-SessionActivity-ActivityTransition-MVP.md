...
- Pool warmup integrado ao ActivitySetup por policy real; `IPoolService` canônico já existe e só deve ser consumido quando houver caso concreto.
...
### 2026-05-22 - Checkpoint congelado - RestartCurrentActivity pós PlayerInput/Movement/Camera/Snapshot

Status formal:

```text
RestartCurrentActivity v0 - CLOSED / PASS funcional atualizado
```

Evidência de smoke grande confirmou que o restart local continua válido depois dos checkpoints recentes de `PlayerInputBindingStage`, `MovementBindingStage`, `CameraBindingStage` e `ActivityObjectSnapshot`.

Regras recongeladas:

```text
RestartCurrentActivity cria nova entrySequence.
RestartCurrentActivity não usa pipeline especial.
A nova execução retorna ao mesmo ActivityEntryPipeline.
RestartCurrentActivity com ActivityContent real executa release/unload/reload.
RestartCurrentActivity no-content executa conclusão semântica sem release físico.
```

Smoke de referência:

```text
activity_01 entrySequence=1
-> RestartCurrentActivity
-> release ActivityScene01 Unloaded
-> activity_01 entrySequence=2
...
```text
faltava recongelar com smoke atual;
não é déficit ativo de lifecycle da SessionActivity.
```
