...
- Historico / compatibilidade apenas.
- O canon ativo do fim de run e `RunEndIntent -> RunResultStage` opcional -> `RunDecision -> Overlay`.
...
- `PostRunOwnershipService`: gate e ownership historicos do rail legado; hoje o contrato canônico deve ser lido como `RunResultStage`/`RunDecision`.
- `PostRunResultService`: snapshot formal do resultado.
- `PostRunOverlayController`: projeção visual de `RunDecision`.
