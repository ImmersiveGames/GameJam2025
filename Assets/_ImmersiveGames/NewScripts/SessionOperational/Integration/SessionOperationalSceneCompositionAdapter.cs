...
namespace _ImmersiveGames.NewScripts.SessionOperational.Integration
{
    ...
    public sealed class SessionOperationalSceneCompositionAdapter : ISessionOperationalRouteTransitionExecutor
    {
        ...
        public async Task<SessionOperationalRouteCompletedFact> ApplyOperationalRouteAsync(SessionOperationalRouteCommand command)
        {
            ...
            DebugUtility.Log(typeof(SessionOperationalSceneCompositionAdapter),
                $"[OBS][SessionOperationalPipeline][Route] adapter='SessionOperationalSceneCompositionAdapter' action='ApplyOperationalRoute' routeIdentity='{routeIdentity}' activeScene='{activeSceneName}' activeSceneKey='{command.ActiveSceneKey.name}' routeOperationId='{command.RouteOperationId}' transitionId='{command.TransitionId}' routeSequence='{command.RouteSequence}' completionHandoff='{command.CompletionHandoff}' source='{source}' reason='{reason}'.",
        ...
        private static string ResolveSceneName(SceneKeyAsset sceneKey, string fieldName)
        {
            if (sceneKey == null)
            {
                HardFailFastH1.Trigger(typeof(SessionOperationalSceneCompositionAdapter),
        ...
        private static void ValidatePersistentScenesPolicyOrFail(SessionOperationalRouteCommand command)
        {
            if (command.Route == null)
            {
                HardFailFastH1.Trigger(
                    typeof(SessionOperationalSceneCompositionAdapter),
