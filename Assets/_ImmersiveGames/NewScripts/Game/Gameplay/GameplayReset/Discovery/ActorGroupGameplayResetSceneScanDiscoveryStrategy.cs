...
namespace _ImmersiveGames.NewScripts.Game.Gameplay.GameplayReset.Discovery
{
    public sealed class ActorGroupGameplayResetSceneScanDiscoveryStrategy : IActorGroupGameplayResetDiscoveryStrategy
    {
        ...
        public ActorGroupGameplayResetSceneScanDiscoveryStrategy(string sceneName)
        {
        ...
        public int CollectTargets(ActorGroupGameplayResetRequest request, List<IActor> results, out bool fallbackUsed)
        {
            ...
            if (request is { Target: ActorGroupGameplayResetTarget.ActorIdSet, ActorIds: { Count: > 0 } })
            {
            ...
            bool isKindTarget = request.Target == ActorGroupGameplayResetTarget.ByActorKind;
            ...
            for (int i = 0; i < behaviours.Length; i++)
            {
                ...
                if (request.Target == ActorGroupGameplayResetTarget.ActorIdSet)
                {
                ...
                {
                    if (!ActorKindMatchRules.MatchesActorKind(actor, requestedKind))
                    {
                        continue;
                    }
                }
