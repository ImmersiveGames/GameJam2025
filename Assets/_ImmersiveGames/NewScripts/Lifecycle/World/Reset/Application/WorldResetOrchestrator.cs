...
namespace _ImmersiveGames.NewScripts.Lifecycle.World.Reset.Application
{
    ...
    {
        private readonly IResetPolicy _policy;
        private readonly IReadOnlyList<IResetGuard> _guards;
        ...
        public WorldResetOrchestrator(
            IResetPolicy policy,
            IReadOnlyList<IResetGuard> guards,
            WorldResetValidationPipeline validation,
            WorldResetExecutor executor)
        {
            _policy = policy;
            _guards = guards ?? Array.Empty<IResetGuard>();
