...
namespace _ImmersiveGames.NewScripts.Bridges.LegacySceneFlow
{
    ...
    {
        private readonly Scripts.Utils.BusEventSystems.EventBinding<LegacyStarted> _transitionStartedBinding;
        private readonly Scripts.Utils.BusEventSystems.EventBinding<LegacyScenesReady> _transitionScenesReadyBinding;
        private readonly Scripts.Utils.BusEventSystems.EventBinding<LegacyCompleted> _transitionCompletedBinding;
        ...
        public LegacySceneFlowBridge()
        {
            _transitionStartedBinding =
                new Scripts.Utils.BusEventSystems.EventBinding<LegacyStarted>(OnLegacySceneTransitionStarted);
            _transitionScenesReadyBinding =
                new Scripts.Utils.BusEventSystems.EventBinding<LegacyScenesReady>(OnLegacySceneTransitionScenesReady);
            _transitionCompletedBinding =
                new Scripts.Utils.BusEventSystems.EventBinding<LegacyCompleted>(OnLegacySceneTransitionCompleted);
        ...
        private void TryRegisterBindings()
        {
            ...
            try
            {
                Scripts.Utils.BusEventSystems.EventBus<LegacyStarted>.Register(_transitionStartedBinding);
                Scripts.Utils.BusEventSystems.EventBus<LegacyScenesReady>.Register(_transitionScenesReadyBinding);
                Scripts.Utils.BusEventSystems.EventBus<LegacyCompleted>.Register(_transitionCompletedBinding);
        ...
        private void UnregisterBindings()
        {
            ...
            Scripts.Utils.BusEventSystems.EventBus<LegacyStarted>.Unregister(_transitionStartedBinding);
            Scripts.Utils.BusEventSystems.EventBus<LegacyScenesReady>.Unregister(_transitionScenesReadyBinding);
            Scripts.Utils.BusEventSystems.EventBus<LegacyCompleted>.Unregister(_transitionCompletedBinding);
            _bindingsRegistered = false;
        ...
        private void OnLegacySceneTransitionStarted(LegacyStarted evt)
        {
            ...
            Infrastructure.Events.EventBus<NewStarted>.Raise(new NewStarted(context));
        ...
        {
            var context = BuildContext(evt.Context);
            Infrastructure.Events.EventBus<NewScenesReady>.Raise(new NewScenesReady(context));
        ...
        {
            var context = BuildContext(evt.Context);
            Infrastructure.Events.EventBus<NewCompleted>.Raise(new NewCompleted(context));
        ...
        private static NewContext BuildContext(object legacyContext)
        {
            ...
            var scenesToLoad = ToStringList(
                ...
                (IEnumerable<string>)TryGetMember<List<string>>(legacyContext, "scenesToLoad", "ScenesToLoad") ??
                Array.Empty<string>());

            var scenesToUnload = ToStringList(
                TryGetMember<IEnumerable<string>>(legacyContext, "scenesToUnload", "ScenesToUnload") ??
                (IEnumerable<string>)TryGetMember<List<string>>(legacyContext, "scenesToUnload", "ScenesToUnload") ??
                Array.Empty<string>());
