...
using UnityEngine;
namespace _ImmersiveGames.NewScripts.Runtime.SceneFlow.Loading.Drivers
{
    public sealed class SceneFlowLoadingHudDriver : MonoBehaviour
    {
        ...
        public void StartLoading(string contextSignature)
        {
            ...
            DebugUtility.LogVerbose<SceneFlowLoadingHudDriver>($"[OBS][LoadingHud] LoadingStarted signature={_lastContextSignature ?? contextSignature}");
        ...
        public void SetReady(string contextSignature)
        {
            ...
            DebugUtility.LogVerbose<SceneFlowLoadingHudDriver>($"[OBS][LoadingHud] LoadingReady signature={_lastContextSignature ?? contextSignature}");
            OnLoadingHudReady?.Invoke(_lastContextSignature ?? contextSignature);
        }

        public Task WaitReadyAsync(string contextSignature)
        {
            var used = string.IsNullOrEmpty(contextSignature) || contextSignature == "no-signature"
                ? _lastContextSignature ?? "no-signature"
                : contextSignature;
            DebugUtility.LogVerbose<SceneFlowLoadingHudDriver>($"[LoadingHud] WaitReadyAsync called signature={used}");
