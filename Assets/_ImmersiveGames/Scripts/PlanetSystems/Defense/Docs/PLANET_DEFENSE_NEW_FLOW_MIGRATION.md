...
- **Editor**: configure cada planeta com uma lista de `PlanetDefenseEntrySo` e escolha o `DefenseChoiceMode` (Random ou Sequential). Cada entrada referencia `WavePresetSo` por role detectado e um preset default obrigatório.
...
## Boas práticas
- **SRP primeiro**: `PlanetDefenseEntrySo` apenas mapeia role → preset e offset; `WavePresetSo` guarda dados da onda; lógica de spawn fica no serviço/runner.
...
## Exemplos rápidos
- **Planeta com comportamento único**: lista com uma `PlanetDefenseEntrySo`, `DefenseChoiceMode.Sequential`. Apenas o default preenchido, `EntryBindByRole` vazio.
...
2. **Preparar assets do novo fluxo**
   - Crie `PlanetDefenseEntrySo` com default obrigatório e binds necessários, referenciando `WavePresetSo` existentes.
   - Confirme `PoolData`, contagem e intervalos em cada `WavePresetSo` (fail-fast no `OnValidate`).
3. **Atualizar fixtures**
   - Substitua mocks/loadouts por coleções de `PlanetDefenseEntrySo` + `DefenseChoiceMode` injetados no orquestrador.
