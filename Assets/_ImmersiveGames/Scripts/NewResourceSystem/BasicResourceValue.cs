...
namespace _ImmersiveGames.Scripts.NewResourceSystem
{
    public class BasicResourceValue: IResourceValue
    {
        ...
        public void Increase(float amount)
        {
            ...
            DebugUtility.LogVerbose<ResourceValueService>($"Increase: Amount={amount:F2}, NewValue={CurrentValue:F2}");
        }

        public void Decrease(float amount)
        {
            CurrentValue = Mathf.Clamp(CurrentValue - amount, 0f, MaxValue);
            DebugUtility.LogVerbose<ResourceValueService>($"Decrease: Amount={amount:F2}, NewValue={CurrentValue:F2}");
        }
