...
namespace _ImmersiveGames.Scripts.PlayerControllerSystem.Movement
{
    ...
    public class OldPlayerMovementController : MonoBehaviour, IResetInterfaces, IResetScopeFilter, IResetOrder
    {
        ...
        // NEW: dom�nio para obter spawn pose
        ...
        // Movement cedo para zerar f�sica e input antes de outros rebinds
        ...
        private void Awake()
        {
            ...
            {
                DebugUtility.LogError<OldPlayerMovementController>("OldCameraResolverService n�o encontrado.");
            }
        ...
        {
            // Zera entradas e �para� f�sica para evitar drift p�s-reset.
            _moveInput = Vector2.zero;
            _lookInput = Vector2.zero;
        ...
        public Task Reset_RestoreAsync(ResetContext ctx)
        {
            ...
            if (_actor != null && _playerDomain != null && !string.IsNullOrWhiteSpace(_actor.ActorId))
            {
                ...
                {
                    // Seguran�a: zera f�sica antes de teleport
                    if (_rb != null)
                    {
                        _rb.linearVelocity = Vector3.zero;
                        _rb.angularVelocity = Vector3.zero;

                        // Preferir �teleport� direto para reset (evita interpola��o residual)
                ...
                else
                {
                    DebugUtility.LogWarning<OldPlayerMovementController>(
                        $"[Reset] SpawnPose n�o encontrado no PlayerDomain para ActorId='{_actor.ActorId}'.", this);
                }
            }

            // Reabilita input e resolve c�mera atual.
            BindInput();
            ResolveAndSetCamera();
        ...
        {
            // Rebind de eventos e c�mera (por seguran�a caso DI/c�mera mude durante reset).
            if (_cameraResolver == null)
            {
