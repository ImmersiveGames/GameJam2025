        ...
        public string GenerateId(GameObject owner, string prefix = null)
        {
            ...
            if (actor != null)
            {
                ...
                if (ownActor != null)
                {
                    ...
                    DebugUtility.LogVerbose<OldUniqueIdFactory>($"Generating new ID for main actor {owner.name}.");
                ...
                else if (!string.IsNullOrEmpty(actor.ActorId))
                {
                    ...
                    DebugUtility.LogVerbose<OldUniqueIdFactory>($"Reusing parent ActorId '{actorId}' for child {owner.name}.");
                }
                else
                {
                    // Fallback se parent ActorId vazio (deve ser raro)
                    DebugUtility.LogWarning<OldUniqueIdFactory>($"Fallback to empty ActorId for {owner.name} (parent ActorId empty).");
                }
            }
            else
            {
                // Non-actor: gerar como obj
                DebugUtility.LogWarning<OldUniqueIdFactory>($"No IActor found for {owner.name}. Using object name as base.");
                _instanceCounts.TryAdd(baseActorName, 0);
                int instanceId = _instanceCounts[baseActorName]++;
            ...
            DebugUtility.LogVerbose<OldUniqueIdFactory>($"Generated ID: {uniqueId} (baseActorName={baseActorName}, baseId={prefix})");
            return uniqueId;
        }
