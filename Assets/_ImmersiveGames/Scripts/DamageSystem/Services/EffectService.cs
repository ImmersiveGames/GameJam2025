// _ImmersiveGames/Scripts/DamageSystem/Services/EffectService.cs
using System;
...
namespace _ImmersiveGames.Scripts.DamageSystem.Services
{
    public class EffectService
    {
        ...
        public void SpawnHitEffect(DamageType damageType, Vector3 position)
        {
            ...
            var poolName = damageType.ToString(); // customize mapping if necessary

            var pool = _poolManager?.GetPool(poolName);
            if (pool != null)
            {
                var poolable = pool.GetObject(position, null, null, true);
                if (poolable != null)
                {
                    var cfg = poolable.GetData<PoolableObjectData>();
                    var lifetime = cfg?.Lifetime ?? 2f;
        ...
        public GameObject SpawnEffect(GameObject prefab, Vector3 position, Quaternion rotation, float? lifetime = null)
        {
            ...
            if (poolable != null)
            {
                var poolName = prefab.name.Replace("(Clone)", "").Trim();
                var pool = _poolManager.GetPool(poolName);
                if (pool != null)
                {
                    var obj = pool.GetObject(position, null, null, true);
                    if (obj != null)
                    {
                        var cfg = obj.GetData<PoolableObjectData>();
                        var life = lifetime ?? cfg?.Lifetime ?? 2f;
